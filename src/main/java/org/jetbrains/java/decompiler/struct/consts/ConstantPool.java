...
public class ConstantPool implements NewClassNameBuilder {
        ...
        for (int i = 1; i < size; i++) {
            ...
            switch (tag) {
                ...
                case CodeConstants.CONSTANT_Integer:
                    pool.add(
                            new PrimitiveConstant(
                                    CodeConstants.CONSTANT_Integer, Integer.valueOf(in.readInt())));
                    break;
    ...
    public String[] getClassElement(
            int elementType, String className, int nameIndex, int descriptorIndex) {
        ...
        if (interceptor != null) {
            ...
            String newElement =
                    interceptor.getName(className + ' ' + elementName + ' ' + descriptor);
            if (newElement != null) {
    ...
    public LinkConstant getLinkConstant(int index) {
        ...
        if (ln != null
            ...
            String newElement =
                    interceptor.getName(ln.classname + ' ' + ln.elementname + ' ' + ln.descriptor);
            String newDescriptor =
                    buildNewDescriptor(ln.type == CodeConstants.CONSTANT_Fieldref, ln.descriptor);
            // TODO: Fix newElement being null caused by ln.classname being a leaf class instead of
            // the class that declared the field/method.
