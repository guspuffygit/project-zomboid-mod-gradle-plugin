...
public class MatchEngine {
    ...
    public MatchEngine(String description) {
        ...
        for (String line : lines) {
            List<String> properties =
                    new ArrayList<>(
                            Arrays.asList(
                                    line.split("\\s+"))); // split on any number of whitespaces
            if (properties.get(0).isEmpty()) {
                properties.remove(0);
            }

            int node_type =
                    "statement".equals(properties.get(0))
    ...
    private boolean match(MatchNode matchNode, IMatchable object) {
        ...
        for (MatchNode childNode : matchNode.getChildren()) {
            ...
            IMatchable childObject =
                    object.findObject(childNode, isStatement ? stat_index : expr_index);
