...
public class ConverterHelper implements IIdentifierRenamer {
    private static final Set<String> KEYWORDS =
            new HashSet<>(
                    Arrays.asList(
                            ...
                            "assert",
                            "enum"));
    private static final Set<String> RESERVED_WINDOWS_NAMESPACE =
            new HashSet<>(
                    Arrays.asList(
    ...
    public boolean toBeRenamed(
            Type elementType, String className, String element, String descriptor) {
    ...
    /**
     * Return {@code true} if, and only if identifier passed is compliant to JLS9 section 3.8 AND
     * DOES NOT CONTAINS so-called "ignorable" characters. Ignorable characters are removed by javac
     * silently during compilation and thus may appear only in specially crafted obfuscated classes.
     * For more information about "ignorable" characters see <a
     * href="https://bugs.openjdk.java.net/browse/JDK-7144981">JDK-7144981</a>.
     *
     * @param identifier Identifier to be checked
     * @return {@code true} in case {@code identifier} passed can be used as an identifier; {@code
     *     false} otherwise.
     */
    private static boolean isValidIdentifier(boolean isMethod, String identifier) {
        ...
        if (isMethod
                && (identifier.equals(CodeConstants.INIT_NAME)
                        || identifier.equals(CodeConstants.CLINIT_NAME))) {
            return true;
        }
