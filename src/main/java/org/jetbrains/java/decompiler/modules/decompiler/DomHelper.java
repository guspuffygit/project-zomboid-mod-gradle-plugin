...
public final class DomHelper {
    ...
    private static RootStatement graphToStatement(ControlFlowGraph graph) {
        ...
        Statement general;
        if (stats.size() > 1
                || firstblock.isSuccessor(
                        firstblock)) { // multiple basic blocks or an infinite loop of one block
            ...
            for (BasicBlock succ : block.getSuccs()) {
                ...
                stat.addSuccessor(
                        new StatEdge(
                                type,
                                stat,
                                (type == StatEdge.TYPE_CONTINUE) ? general : stsucc,
    ...
    public static VBStyleCollection<List<Integer>, Integer> calcPostDominators(
            Statement container) {
        ...
        List<Statement> lstStats =
                container.getPostReversePostOrderList(
                        StrongConnectivityHelper.getExitReps(components));
        ...
        do {
            ...
            for (Statement stat : lstStats) {
                ...
                List<Statement> lstSuccs =
                        stat.getNeighbours(StatEdge.TYPE_REGULAR, Statement.DIRECTION_FORWARD);
                ...
                if (!InterpreterUtil.equalObjects(domsSuccs, doms)) {
                    ...
                    List<Statement> lstPreds =
                            stat.getNeighbours(StatEdge.TYPE_REGULAR, Statement.DIRECTION_BACKWARD);
    ...
    public static RootStatement parseGraph(ControlFlowGraph graph) {
        ...
        if (!processStatement(root, new HashMap<>())) {
            //			try {
            //				DotExporter.toDotFile(root.getFirst().getStats().get(13), new
            // File("c:\\Temp\\stat1.dot"));
            //			} catch (Exception ex) {
            //				ex.printStackTrace();
            //			}
                ...
                for (int i = 0; i < lst.size() - 1; i++) {
                    ...
                    if (current.isMonitorEnter()) {
                        ...
                        if (next.type == Statement.TYPE_CATCHALL) {
                            ...
                            if (ca.getFirst().isContainsMonitorExit()
                                    && ca.getHandler().isContainsMonitorExit()) {
                                // remove the head block from sequence
                                current.removeSuccessor(
                                        current.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)
                                                .get(0));

                                for (StatEdge edge :
                                        current.getPredecessorEdges(
                                                Statement.STATEDGE_DIRECT_ALL)) {
                                    current.removePredecessor(edge);
                                    edge.getSource()
                                            .changeEdgeNode(
                                                    Statement.DIRECTION_FORWARD, edge, nextDirect);
                                ...
                                SynchronizedStatement sync =
                                        new SynchronizedStatement(
                                                current, ca.getFirst(), ca.getHandler());
                                sync.setAllParent();
                                ...
                                current.addSuccessor(
                                        new StatEdge(
                                                StatEdge.TYPE_REGULAR, current, ca.getFirst()));
    ...
    private static boolean processStatement(
            Statement general, HashMap<Integer, Set<Integer>> mapExtPost) {
        ...
        for (int mapstage = 0; mapstage < 2; mapstage++) {
            for (int reducibility = 0;
                    reducibility < 5;
                    reducibility++) { // FIXME: implement proper node splitting. For now up to 5
                // nodes in sequence are splitted.
                    ...
                    if (IrreducibleCFGDeobfuscator.isStatementIrreducible(general)) {
                        if (!IrreducibleCFGDeobfuscator.splitIrreducibleNode(general)) {
                            DecompilerContext.getLogger()
                                    .writeMessage(
                                            "Irreducible statement cannot be decomposed!",
    ...
    private static Statement findGeneralStatement(
            Statement stat, boolean forceall, HashMap<Integer, Set<Integer>> mapExtPost) {
                    ...
                    for (Statement handler : setHandlers) {
                        ...
                        if (!addhd) {
                            List<Statement> hdsupp =
                                    handler.getNeighbours(
                                            StatEdge.TYPE_EXCEPTION, Statement.DIRECTION_BACKWARD);
                            addhd =
                                    (setNodes.containsAll(hdsupp)
                                            && (setNodes.size() > hdsupp.size()
    ...
    private static boolean findSimpleStatements(
            Statement stat, HashMap<Integer, Set<Integer>> mapExtPost) {
