...
public class InvocationExprent extends Exprent {
    ...
    public InvocationExprent(
        ...
        if (opcode == CodeConstants.opc_invokedynamic) {
            ...
            if (dynamicInvocationType == CodeConstants.CONSTANT_MethodHandle_REF_invokeStatic) {
                ...
                // FIXME: remove the first parameter completely from the list. It's the object type
                // for a virtual lambda method.
    ...
    public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
        ...
        if (isStatic) {
            ...
            if (instance != null && instance.type == Exprent.EXPRENT_VAR) {
                ...
                if (this_classname != null) {
                    ...
                    if (invocationTyp == INVOKE_SPECIAL) {
                        if (!classname.equals(
                                this_classname)) { // TODO: direct comparison to the super class?
                            StructClass cl =
                                    DecompilerContext.getStructContext().getClass(classname);
                            boolean isInterface =
                                    cl != null && cl.hasModifier(CodeConstants.ACC_INTERFACE);
            ...
            if (functype == TYP_GENERAL) {
                if (super_qualifier != null) {
                    ...
                    if (isUnboxingCall()) {
                        // we don't print the unboxing call - no need to bother with the instance
                        // wrapping / casting
                        buf.append(res);
                        return buf;
                    }
            ...
            case TYP_INIT:
                if (super_qualifier != null) {
                    ...
                    throw new RuntimeException(
                            "Unrecognized invocation of " + CodeConstants.INIT_NAME);
                }
        }
        ...
        if (functype == TYP_INIT) {
            ClassNode newNode =
                    DecompilerContext.getClassProcessor().getMapRootClasses().get(classname);
            if (newNode != null) {
                mask =
                        ExprUtil.getSyntheticParametersMask(
                                newNode, stringDescriptor, lstParameters.size());
                isEnum =
                        newNode.classStruct.hasModifier(CodeConstants.ACC_ENUM)
        ...
        for (int i = start; i < lstParameters.size(); i++) {
            if (mask == null || mask.get(i) == null) {
                ...
                // 'byte' and 'short' literals need an explicit narrowing type cast when used as a
                // parameter
                ExprProcessor.getCastedExprent(
                        lstParameters.get(i),
                        descriptor.params[i],
    ...
    public boolean isBoxingCall() {
        if (isStatic && "valueOf".equals(name) && lstParameters.size() == 1) {
            ...
            if (lstParameters.get(0).type == Exprent.EXPRENT_CONST) {
                ...
                if (lstParameters.get(0).getExprType().typeFamily
                        == CodeConstants.TYPE_FAMILY_INTEGER) {
                    if (classname.equals("java/lang/Integer")) {
                        return true;
                    }
                }

                if (paramType == CodeConstants.TYPE_BYTECHAR
                        || paramType == CodeConstants.TYPE_SHORTCHAR) {
    ...
    private boolean isUnboxingCall() {
        return !isStatic
                && lstParameters.size() == 0
                && classname.equals(UNBOXING_METHODS.get(name));
    }
    ...
    public boolean match(MatchNode matchNode, MatchEngine engine) {
        ...
        for (Entry<MatchProperties, RuleValue> rule : matchNode.getRules().entrySet()) {
            ...
            if (key == MatchProperties.EXPRENT_INVOCATION_PARAMETER) {
                if (value.isVariable()
                        && (value.parameter >= lstParameters.size()
