...
public class FunctionExprent extends Exprent {
    ...
    private static final Set<Integer> ASSOCIATIVITY =
            new HashSet<>(
                    Arrays.asList(
                            FUNCTION_ADD,
                            FUNCTION_MUL,
    ...
    public VarType getExprType() {
        ...
        if (funcType <= FUNCTION_NEG
            ...
            switch (funcType) {
                ...
                case FUNCTION_XOR:
                    if (type1.type == CodeConstants.TYPE_BOOLEAN
                            & type2.type == CodeConstants.TYPE_BOOLEAN) {
                        exprType = VarType.VARTYPE_BOOLEAN;
                    } else {
                        exprType = getMaxVarType(new VarType[] {type1, type2});
                    }
            ...
            VarType supertype =
                    VarType.getCommonSupertype(param1.getExprType(), param2.getExprType());
        ...
        switch (funcType) {
            case FUNCTION_IIF:
                ...
                result.addMinTypeExprent(
                        lstOperands.get(2), VarType.getMinTypeInFamily(supertype.typeFamily));
                break;
            case FUNCTION_I2L:
            case FUNCTION_I2F:
            case FUNCTION_I2D:
                        ...
                        if (type2.isStrictSuperset(type1)) {
                            ...
                            boolean param1_false_boolean =
                                    type1.isFalseBoolean()
                                            || (param1.type == Exprent.EXPRENT_CONST
    ...
    public boolean equals(Object o) {
        ...
        return funcType == fe.getFuncType()
                && InterpreterUtil.equalLists(
                        lstOperands, fe.getLstOperands()); // TODO: order of operands insignificant
    }
        ...
        switch (funcType) {
            ...
            case FUNCTION_ARRAY_LENGTH:
                ...
                if (arr.getExprType().arrayDim == 0) {
                    VarType objArr =
                            VarType.VARTYPE_OBJECT.resizeArrayDim(1); // type family does not change
    ...
    private TextBuffer wrapOperandString(
            Exprent expr, boolean eq, int indent, BytecodeMappingTracer tracer) {
    ...
    private static VarType getMaxVarType(VarType[] arr) {
        int[] types = {
            CodeConstants.TYPE_DOUBLE, CodeConstants.TYPE_FLOAT, CodeConstants.TYPE_LONG
        };
