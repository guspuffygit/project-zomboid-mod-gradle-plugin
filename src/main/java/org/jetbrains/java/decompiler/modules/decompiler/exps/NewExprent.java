...
public class NewExprent extends Exprent {
    ...
    public NewExprent(
            VarType newType, ListStack<Exprent> stack, int arrayDim, Set<Integer> bytecodeOffsets) {
    ...
    public NewExprent(VarType newType, List<Exprent> lstDims, Set<Integer> bytecodeOffsets) {
        ...
        if (newType.type == CodeConstants.TYPE_OBJECT && newType.arrayDim == 0) {
            ClassNode node =
                    DecompilerContext.getClassProcessor().getMapRootClasses().get(newType.value);
            if (node != null
        ...
        if (newType.arrayDim != 0) {
            ...
            if (newType.arrayDim == 1) {
                ...
                for (Exprent element : lstArrayElements) {
                    result.addMinTypeExprent(
                            element, VarType.getMinTypeInFamily(leftType.typeFamily));
    ...
    public List<Exprent> getAllExprents() {
        ...
        if (newType.arrayDim != 0) {
            ...
            if (constructor
                    != null) { // should be true only for a lambda expression with a virtual content
                // method
                lst.add(constructor);
            }
    ...
    public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
        ...
        if (anonymous) {
            ...
            boolean selfReference =
                    DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS_NODE) == child;

            // IDEA-204310 - avoid backtracking later on for lambdas (causes spurious imports)
            if (!enumConst
                    && (!lambda
                            || DecompilerContext.getOption(
                                    IFernflowerPreferences.LAMBDA_TO_ANONYMOUS_CLASS))) {
                String enclosing = null;

                if (!lambda && constructor != null) {
                    enclosing =
                            getQualifiedNewInstance(
                                    child.anonymousClassType.value,
                ...
                if (selfReference) {
                    ...
                    GenericClassDescriptor descriptor =
                            ClassWriter.getGenericClassDescriptor(child.classStruct);
                    if (descriptor != null) {
            ...
            if (!lambda && constructor != null) {
                ...
                List<VarVersionPair> mask =
                        child.getWrapper()
                                .getMethodWrapper(
                                        CodeConstants.INIT_NAME, constructor.getStringDescriptor())
                                .synthParameters;
                if (mask == null) {
                    InvocationExprent superCall = child.superInvocation;
                    mask =
                            ExprUtil.getSyntheticParametersMask(
                                    superCall.getClassname(),
                                    superCall.getStringDescriptor(),
                                    parameters.size());
                }
                ...
                for (int i = start; i < parameters.size(); i++) {
                    if (mask == null || mask.get(i) == null) {
                        ...
                        ExprProcessor.getCastedExprent(
                                parameters.get(i),
                                constructor.getDescriptor().params[i],
                                buf,
                                indent,
                                true,
                                tracer);
            ...
            if (lambda) {
                if (!DecompilerContext.getOption(
                        IFernflowerPreferences.LAMBDA_TO_ANONYMOUS_CLASS)) {
                    buf.setLength(
                            0); // remove the usual 'new <class>()', it will be replaced with lambda
                    // style '() ->'
                }
            ...
            for (int i = 0; i < lstArrayElements.size(); i++) {
                ...
                ExprProcessor.getCastedExprent(
                        lstArrayElements.get(i), leftType, buf, indent, false, tracer);
            }
            buf.append('}');
            ...
            if (!enumConst) {
                ...
                if (constructor != null) {
                    enclosing =
                            getQualifiedNewInstance(
                                    newType.value, constructor.getLstParameters(), indent, tracer);
                    if (enclosing != null) {
                        buf.append(enclosing).append('.');
                    }
                }
                    ...
                    for (int i = start; i < parameters.size(); i++) {
                        if (mask == null || mask.get(i) == null) {
                            ...
                            if (i == parameters.size() - 1
                                    && expr.getExprType() == VarType.VARTYPE_NULL
                                    && probablySyntheticParameter(leftType.value)) {
                            ...
                            ExprProcessor.getCastedExprent(
                                    expr, leftType, buf, indent, true, false, true, true, tracer);
            ...
            // if there is just one element of Object[] type it needs to be casted to resolve
            // ambiguity
    ...
    private static String getQualifiedNewInstance(
            String classname, List<Exprent> lstParams, int indent, BytecodeMappingTracer tracer) {
        ClassNode node = DecompilerContext.getClassProcessor().getMapRootClasses().get(classname);
