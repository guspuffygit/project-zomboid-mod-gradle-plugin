...
public class Exprent implements IMatchable {
    ...
    public Set<Integer> bytecode =
            null; // offsets of bytecode instructions decompiled to this exprent
    ...
    public IMatchable findObject(MatchNode matchNode, int index) {
        ...
        if (position != null) {
            if (position.matches("-?\\d+")) {
                return lstAllExprents.get(
                        (lstAllExprents.size() + Integer.parseInt(position))
    ...
    public boolean match(MatchNode matchNode, MatchEngine engine) {
        ...
        for (Entry<MatchProperties, RuleValue> rule : matchNode.getRules().entrySet()) {
            ...
            if (key == MatchProperties.EXPRENT_TYPE
                    && this.type != (Integer) rule.getValue().value) {
                return false;
            }
            if (key == MatchProperties.EXPRENT_RET
                    && !engine.checkAndSetVariableValue((String) rule.getValue().value, this)) {
                return false;
            }
        }
