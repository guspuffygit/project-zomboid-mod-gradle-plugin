...
public class Statement implements IMatchable {
    ...
    public void collapseNodesToStatement(Statement stat) {
        ...
        for (StatEdge prededge : head.getAllPredecessorEdges()) {
            if (prededge.getType() != StatEdge.TYPE_EXCEPTION
                ...
                prededge.getSource()
                        .changeEdgeType(DIRECTION_FORWARD, prededge, StatEdge.TYPE_CONTINUE);
        ...
        Set<Statement> setHandlers =
                new HashSet<>(head.getNeighbours(StatEdge.TYPE_EXCEPTION, DIRECTION_FORWARD));
    ...
    private void addEdgeDirectInternal(int direction, StatEdge edge, int edgetype) {
        ...
        mapStates
                .computeIfAbsent(edgetype, k -> new ArrayList<>())
    ...
    public void replaceStatement(Statement oldstat, Statement newstat) {
        ...
        for (int i = lst.size() - 1; i >= 0; i--) {
            ...
            if (edge.getSource() != newstat) {
                ...
                if (this == edge.getDestination()
                        || this.containsStatementStrict(edge.getDestination())) {
                    edge.closure = null;
                } else {
    ...
    private static void addToReversePostOrderListIterative(
            Statement root, List<? super Statement> lst) {
            ...
            for (; index < lstEdges.size(); index++) {
                ...
                if (!setVisited.contains(succ)
                        && (edge.getType() == StatEdge.TYPE_REGULAR
                                || edge.getType()
                                        == StatEdge.TYPE_EXCEPTION)) { // TODO: edge filter?
    ...
    private static void addToPostReversePostOrderList(
            Statement stat, List<? super Statement> lst, HashSet<? super Statement> setVisited) {
        ...
        for (StatEdge prededge :
                stat.getEdges(
                        StatEdge.TYPE_REGULAR | StatEdge.TYPE_EXCEPTION, DIRECTION_BACKWARD)) {
