...
public class GenericDominatorEngine {
    ...
    private final VBStyleCollection<IGraphNode, IGraphNode> colOrderedIDoms =
    ...
    private static IGraphNode getCommonIDom(
            IGraphNode node1,
            IGraphNode node2,
    ...
    private void calcIDoms() {
        ...
        while (true) {
            ...
            for (IGraphNode node : lstNodes) {
                ...
                if (!idom.equals(
                        oldidom)) { // oldidom is null iff the node is touched for the first time
                    changed = true;
                }
            }
