...
public final class ExceptionDeobfuscator {
    ...
    public static void restorePopRanges(ControlFlowGraph graph) {
        ...
        for (ExceptionRangeCFG range : graph.getExceptions()) {
            ...
            for (Range arr : lstRanges) {
                if (arr.handler == range.getHandler()
                        && InterpreterUtil.equalObjects(
                                range.getUniqueExceptionsString(), arr.uniqueStr)) {
                    ...
                    if (firstinstr.opcode == CodeConstants.opc_pop
                        ...
                        for (Range range_super : lstRanges) { // finally or strict superset
                            ...
                            if (range != range_super) {
                                Set<BasicBlock> setrange_super =
                                        new HashSet<>(range_super.protectedRange);

                                if (!setrange.contains(range_super.handler)
                                        && !setrange_super.contains(handler)
                                        && (range_super.uniqueStr == null
                                                || setrange_super.containsAll(setrange))) {
                                    ...
                                    if (!setrange_super.isEmpty()) {
                                        ...
                                        if (seq.length() > 1) {
                                            ...
                                            InstructionSequence newseq =
                                                    new SimpleInstructionSequence();
                                            ...
                                            // replace handler
                                            for (ExceptionRangeCFG range_ext :
                                                    graph.getExceptions()) {
                                                if (range_ext.getHandler() == handler) {
                                                    range_ext.setHandler(newblock);
                                                        ...
                                                        .getProtectedRange()
                                                        .contains(handler)) {
                                                    newblock.addSuccessorException(
                                                            range_ext.getHandler());
    ...
    public static void removeCircularRanges(final ControlFlowGraph graph) {
        ...
        for (int i = lstRanges.size() - 1; i >= 0; i--) {
            ...
            if (rangeList.contains(handler)) { // TODO: better removing strategy
                ...
                List<BasicBlock> lstRemBlocks =
                        getReachableBlocksRestricted(range.getHandler(), range, engine);
    ...
    private static List<BasicBlock> getReachableBlocksRestricted(
            BasicBlock start, ExceptionRangeCFG range, GenericDominatorEngine engine) {
    ...
    public static boolean hasObfuscatedExceptions(ControlFlowGraph graph) {
        ...
        for (ExceptionRangeCFG range : graph.getExceptions()) {
            mapRanges
                    .computeIfAbsent(range.getHandler(), k -> new HashSet<>())
    ...
    private static boolean splitExceptionRange(
        ...
        for (BasicBlock entry : setEntries) {
            ...
            if (!lstSubrangeBlocks.isEmpty()
                    && lstSubrangeBlocks.size() < range.getProtectedRange().size()) {
                // add new range
                ExceptionRangeCFG subRange =
                        new ExceptionRangeCFG(
                                lstSubrangeBlocks, range.getHandler(), range.getExceptionTypes());
                ...
                DecompilerContext.getLogger()
                        .writeMessage(
                                "Inconsistency found while splitting protected range",
    ...
    public static void insertDummyExceptionHandlerBlocks(
            ControlFlowGraph graph, int bytecode_version) {
