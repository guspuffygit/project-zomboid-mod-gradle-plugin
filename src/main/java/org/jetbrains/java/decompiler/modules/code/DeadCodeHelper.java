...
public final class DeadCodeHelper {
    ...
    private static boolean removeEmptyBlock(
            ControlFlowGraph graph, BasicBlock block, boolean merging) {
            ...
            if (block.getPredExceptions().isEmpty()
                ...
                if (setExits.contains(block)) {
                    ...
                    if (pred.getSuccs().size() != 1
                            || (!pred.getSeq().isEmpty()
                                    && pred.getSeq().getLastInstr().group
                                            == CodeConstants.GROUP_SWITCH)) {
                        return false;
                    }
                }
                ...
                for (int i = lstRanges.size() - 1; i >= 0; i--) {
                    ...
                    if (lst.size() == 1 && lst.get(0) == block) {
                        if (DecompilerContext.getOption(
                                IFernflowerPreferences.REMOVE_EMPTY_RANGES)) {
    ...
    public static void extendSynchronizedRangeToMonitorexit(ControlFlowGraph graph) {
        while (true) {
            ...
            for (ExceptionRangeCFG range : graph.getExceptions()) {
                ...
                if (predSeq.isEmpty()
                        || predSeq.getLastInstr().opcode != CodeConstants.opc_monitorenter) {
                ...
                if (succHandlerSeq.isEmpty()
                        || succHandlerSeq.getLastInstr().opcode != CodeConstants.opc_athrow) {
            ...
            if (len > 0 && len < 3) {
                ...
                if (ok) {
                    if (!block.getPreds().isEmpty()) {
                        ...
                        HashSet<BasicBlock> setRangesToBeRemoved =
                                new HashSet<>(block.getSuccExceptions());
