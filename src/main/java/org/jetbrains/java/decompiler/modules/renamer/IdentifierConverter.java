...
public class IdentifierConverter implements NewClassNameBuilder {
    ...
    public IdentifierConverter(
            StructContext context, IIdentifierRenamer helper, PoolInterceptor interceptor) {
    ...
    private void renameAllClasses() {
        // order not important
        List<ClassWrapperNode> lstAllClasses =
                new ArrayList<>(getReversePostOrderListIterative(rootInterfaces));
    ...
    private void renameClassIdentifiers(StructClass cl, Map<String, String> names) {
        ...
        for (int i = 0; i < methods.size(); i++) {
            ...
            if (!cl.isOwn() || mt.hasModifier(CodeConstants.ACC_NATIVE)) {
            ...
            } else if (helper.toBeRenamed(
                    IIdentifierRenamer.Type.ELEMENT_METHOD,
                    classOldFullName,
                    name,
                    mt.getDescriptor())) {
                ...
                interceptor.addName(
                        classOldFullName + " " + mt.getName() + " " + mt.getDescriptor(),
                        classNewFullName
                                + " "
                                + name
                                + " "
                                + buildNewDescriptor(false, mt.getDescriptor()));
            }
        }
        ...
        for (StructField fd : cl.getFields()) {
            if (helper.toBeRenamed(
                    IIdentifierRenamer.Type.ELEMENT_FIELD,
                    classOldFullName,
                    fd.getName(),
                    fd.getDescriptor())) {
                String newName;
                do {
                    newName =
                            helper.getNextFieldName(
                                    classOldFullName, fd.getName(), fd.getDescriptor());
                ...
                interceptor.addName(
                        ...
                        classNewFullName
                                ...
                                + newName
                                + " "
                                + buildNewDescriptor(true, fd.getDescriptor()));
            }
        }
    }
