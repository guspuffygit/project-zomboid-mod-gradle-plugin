...
public class VarTypeProcessor {
    ...
    private void setInitVars(RootStatement root) {
        ...
        if (thisVar) {
            StructClass cl =
                    (StructClass) DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS);
        ...
        for (int i = 0; i < md.params.length; i++) {
            mapExprentMinTypes.put(
                    new VarVersionPair(varIndex + (thisVar ? 1 : 0), 1), md.params[i]);
            mapExprentMaxTypes.put(
                    new VarVersionPair(varIndex + (thisVar ? 1 : 0), 1), md.params[i]);
        ...
        switch (exprent.type) {
            case Exprent.EXPRENT_CONST:
                ...
                if (newType.typeFamily > CodeConstants.TYPE_FAMILY_INTEGER
                    ...
                    VarType minInteger =
                            new ConstExprent((Integer) constExpr.getValue(), false, null)
                                    .getConstType();
            ...
            case Exprent.EXPRENT_VAR:
                ...
                if (minMax == 0) { // min
                    ...
                    if (currentMinType != null
                            && (newMinType.typeFamily > currentMinType.typeFamily
                    ...
                    case FunctionExprent.FUNCTION_IIF: // FIXME:
                        res =
                                changeExprentType(func.getLstOperands().get(1), newType, minMax)
                                        & changeExprentType(
                                                func.getLstOperands().get(2), newType, minMax);
                        break;
                    case FunctionExprent.FUNCTION_AND:
                    case FunctionExprent.FUNCTION_OR:
                    case FunctionExprent.FUNCTION_XOR:
                        res =
                                changeExprentType(func.getLstOperands().get(0), newType, minMax)
                                        & changeExprentType(
                                                func.getLstOperands().get(1), newType, minMax);
                        break;
                }
        }
