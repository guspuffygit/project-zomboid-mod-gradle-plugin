...
public final class SecondaryFunctionsHelper {
    ...
    private static final int[] funcsnot =
            new int[] {
    ...
    public static boolean identifySecondaryFunctions(Statement stat, VarProcessor varProc) {
        if (stat.getExprents() == null) {
            ...
            if (stat.type == Statement.TYPE_IF) {
                ...
                if (ifelsestat.iftype == IfStatement.IFTYPE_IFELSE
                        && ifstat.getExprents() != null
                        && ifstat.getExprents().isEmpty()
                        && (ifstat.getAllSuccessorEdges().isEmpty()
                                || !ifstat.getAllSuccessorEdges().get(0).explicit)) {
                    ...
                    if (ifelsestat.getAllSuccessorEdges().isEmpty()
                            && !ifstat.getAllSuccessorEdges().isEmpty()) {
    ...
    private static Exprent identifySecondaryFunctions(
            Exprent exprent, boolean statement_level, VarProcessor varProc) {
        if (exprent.type == Exprent.EXPRENT_FUNCTION) {
            ...
            switch (fexpr.getFuncType()) {
                ...
                case FunctionExprent.FUNCTION_LE:
                    ...
                    if (expr1.type == Exprent.EXPRENT_FUNCTION
                            && expr2.type == Exprent.EXPRENT_CONST) {
                        ...
                        if (functype == FunctionExprent.FUNCTION_LCMP
                            ...
                            if (desttype >= 0) {
                                if (functype != FunctionExprent.FUNCTION_LCMP) {
                                    boolean oneForNan =
                                            functype == FunctionExprent.FUNCTION_DCMPL
                                                    || functype == FunctionExprent.FUNCTION_FCMPL;
                                    boolean trueForOne =
                                            desttype == FunctionExprent.FUNCTION_LT
                                ...
                                return new FunctionExprent(
                                        desttype, funcexpr.getLstOperands(), funcexpr.bytecode);
                            }
                        }
                    }
            }
        ...
        switch (exprent.type) {
                ...
                switch (fexpr.getFuncType()) {
                        ...
                        for (int i = 0; i < 2; i++) {
                            ...
                            if (operand.type == Exprent.EXPRENT_CONST
                                ...
                                if (val == -1) {
                                    ...
                                    return new FunctionExprent(
                                            FunctionExprent.FUNCTION_BIT_NOT,
                                            lstBitNotOperand,
                                            fexpr.bytecode);
                                }
                            }
                    ...
                    case FunctionExprent.FUNCTION_NE:
                        if (lstOperands.get(0).getExprType().type == CodeConstants.TYPE_BOOLEAN
                                && lstOperands.get(1).getExprType().type
                            ...
                            for (int i = 0; i < 2; i++) {
                                if (lstOperands.get(i).type == Exprent.EXPRENT_CONST) {
                                    ...
                                    if ((fexpr.getFuncType() == FunctionExprent.FUNCTION_EQ
                                                    && val == 1)
                                            || (fexpr.getFuncType() == FunctionExprent.FUNCTION_NE
                                                    && val == 0)) {
                    ...
                    case FunctionExprent.FUNCTION_DCMPG:
                                ...
                                new FunctionExprent(
                                        ...
                                        Arrays.asList(
                                                new FunctionExprent(
                                                        FunctionExprent.FUNCTION_LT,
                                                        Arrays.asList(
                                                                new VarExprent(var, type, varProc),
                                                                ConstExprent.getZeroConstant(
                                                                        type.type)),
                                                        null),
                        ...
                        return new FunctionExprent(
                                FunctionExprent.FUNCTION_IIF,
                                Arrays.asList(
                                        head, new ConstExprent(VarType.VARTYPE_INT, 0, null), iff),
                                fexpr.bytecode);
                }
                break;
            ...
            case Exprent.EXPRENT_INVOCATION:
                if (!statement_level) { // simplify if exprent is a real expression. The opposite
                    // case is pretty absurd, can still happen however (and
                        ...
                        case FunctionExprent.FUNCTION_LE:
                            if (!canSimplify) {
                                ...
                                if (commonSupertype != null) {
                                    canSimplify =
                                            commonSupertype.type != CodeConstants.TYPE_FLOAT
