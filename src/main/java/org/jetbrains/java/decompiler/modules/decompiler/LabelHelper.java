...
public final class LabelHelper {
    ...
    private static void removeNonImmediateEdges(Statement stat) {
        ...
        if (!stat.hasBasicSuccEdge()) {
            for (StatEdge edge :
                    stat.getSuccessorEdges(StatEdge.TYPE_CONTINUE | StatEdge.TYPE_BREAK)) {
    ...
    public static void lowContinueLabels(Statement stat, HashSet<StatEdge> edges) {
        ...
        if (!ok) {
            ...
            ok =
                    dostat.getLooptype() == DoStatement.LOOP_DO
                            || dostat.getLooptype() == DoStatement.LOOP_WHILE
                            || (dostat.getLooptype() == DoStatement.LOOP_FOR
                                    && dostat.getIncExprent() == null);
        }
    ...
    private static void setRetEdgesUnlabeled(RootStatement root) {
        ...
        for (StatEdge edge : exit.getAllPredecessorEdges()) {
            ...
            if (edge.getType() == StatEdge.TYPE_FINALLYEXIT
                    || (lst != null
                            && !lst.isEmpty()
    ...
    private static HashMap<Statement, List<StatEdge>> setExplicitEdges(Statement stat) {
        ...
        switch (stat.type) {
            ...
            case Statement.TYPE_CATCHALL:
                for (Statement st : stat.getStats()) {
                    ...
                    if (stat.type == Statement.TYPE_TRYCATCH
                            || st == stat.getFirst()) { // edges leaving a finally catch block
                        // are always explicit
                        // merge the maps
                        if (mapEdges1 != null) {
            ...
            case Statement.TYPE_ROOT:
                mapEdges = setExplicitEdges(stat.getFirst());
                processEdgesWithNext(
                        stat.getFirst(), mapEdges, ((RootStatement) stat).getDummyExit());
                break;
    ...
    private static void processEdgesWithNext(
            Statement stat, HashMap<Statement, List<StatEdge>> mapEdges, Statement next) {
    ...
    private static LabelSets processStatementLabel(Statement stat) {
        ...
        if (stat.getExprents() == null) {
            ...
            boolean shieldType =
                    (stat.type == Statement.TYPE_DO || stat.type == Statement.TYPE_SWITCH);
            if (shieldType) {
                for (StatEdge edge : stat.getLabelEdges()) {
                    if (edge.explicit
                            && ((edge.getType() == StatEdge.TYPE_BREAK
                                            && sets.breaks.contains(edge.getSource()))
            ...
            for (StatEdge edge : lst) {
                if (edge.explicit) {
                    ...
                    if (minclosure != edge.closure
                            && !InlineSingleBlockHelper.isBreakEdgeLabeled(
                                    edge.getSource(), minclosure)) {
                        edge.getSource()
                                .changeEdgeType(
                                        Statement.DIRECTION_FORWARD, edge, StatEdge.TYPE_BREAK);
                        edge.labeled = false;
