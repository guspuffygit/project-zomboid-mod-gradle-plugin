...
public final class ConcatenationHelper {
    ...
    private static final VarType builderType =
            new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang/StringBuilder");
    private static final VarType bufferType =
            new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang/StringBuffer");
    ...
    private static Exprent createConcatExprent(List<Exprent> lstOperands, Set<Integer> bytecode) {
        ...
        for (int i = 1; i < lstOperands.size(); i++) {
            func =
                    new FunctionExprent(
                            FunctionExprent.FUNCTION_STR_CONCAT,
                            Arrays.asList(func, lstOperands.get(i)),
                            bytecode);
        }
    ...
    private static List<Exprent> extractParameters(
            List<PooledConstant> bootstrapArguments, InvocationExprent expr) {
                ...
                for (int i = 0; i < recipe.length(); i++) {
                    ...
                    if (c == TAG_CONST || c == TAG_ARG) {
                        ...
                        if (acc.length() > 0) {
                            res.add(
                                    new ConstExprent(
                                            VarType.VARTYPE_STRING, acc.toString(), expr.bytecode));
                            acc.setLength(0);
                        }
                        if (c == TAG_CONST) {
                            // skip for now
    ...
    private static boolean isNewConcat(NewExprent expr, VarType cltype) {
        if (expr.getNewType().equals(cltype)) {
            ...
            return params.length == 0
                    || params.length == 1 && params[0].equals(VarType.VARTYPE_STRING);
        }
