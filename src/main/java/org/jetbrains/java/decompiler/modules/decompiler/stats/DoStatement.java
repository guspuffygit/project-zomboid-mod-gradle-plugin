...
public final class DoStatement extends Statement {
    ...
    public static Statement isHead(Statement head) {
        if (head.getLastBasicType() == LASTBASICTYPE_GENERAL && !head.isMonitorEnter()) {
            ...
            // continues
            if (head.type != TYPE_DO
                    && (edge == null || edge.getType() != StatEdge.TYPE_REGULAR)
    ...
    public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
        ...
        switch (looptype) {
            ...
            case LOOP_DOWHILE:
                ...
                buf.appendIndent(indent)
                        .append("} while(")
                        .append(conditionExprent.get(0).toJava(indent, tracer))
                        .append(");")
                        .appendLineSeparator();
                tracer.incrementCurrentSourceLine();
                break;
            ...
            case LOOP_FOR:
                ...
                buf.append("; ")
                        .append(conditionExprent.get(0).toJava(indent, tracer))
                        .append("; ")
                        .append(incExprent.get(0).toJava(indent, tracer))
                        .append(") {")
                        .appendLineSeparator();
                tracer.incrementCurrentSourceLine();
                buf.append(ExprProcessor.jmpWrapper(first, indent + 1, false, tracer));
