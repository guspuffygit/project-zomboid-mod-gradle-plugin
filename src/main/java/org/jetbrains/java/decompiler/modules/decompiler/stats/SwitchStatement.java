...
public final class SwitchStatement extends Statement {
    ...
    private SwitchStatement(Statement head, Statement poststat) {
        ...
        // find post node
        Set<Statement> lstNodes =
                new HashSet<>(head.getNeighbours(StatEdge.TYPE_REGULAR, DIRECTION_FORWARD));
            ...
            for (int j = 0; j < edges.size(); j++) {
                if (edges.get(j) == default_edge) {
                    ...
                    if (value instanceof FieldExprent
                            && ((FieldExprent) value).isStatic()) { // enum values
    ...
    public void initExprents() {
        SwitchExprent swexpr =
                (SwitchExprent) first.getExprents().remove(first.getExprents().size() - 1);
    ...
    public void sortEdgesAndNodes() {
        ...
        // collect exit edges
        List<StatEdge> lstExitEdges =
                first.getSuccessorEdges(StatEdge.TYPE_BREAK | StatEdge.TYPE_CONTINUE);
        while (!lstExitEdges.isEmpty()) {
            ...
            for (int i = lstExitEdges.size() - 1; i >= 0; i--) {
                ...
                if (edgeTemp.getDestination() == edge.getDestination()
                        && edgeTemp.getType() == edge.getType()) {
        ...
        for (int index = 0; index < nodes.size(); index++) {
            ...
            if (stat != null) {
                ...
                if (!setPreds.isEmpty()) {
                    Statement pred =
                            setPreds.iterator()
                                    .next(); // assumption: at most one predecessor node besides the
                    // head. May not hold true for obfuscated code.
