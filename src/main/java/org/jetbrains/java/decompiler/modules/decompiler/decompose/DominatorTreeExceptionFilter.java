    ...
    private void buildExceptionRanges() {
        for (Statement stat : statement.getStats()) {
            List<Statement> lstPreds =
                    stat.getNeighbours(StatEdge.TYPE_EXCEPTION, Statement.DIRECTION_BACKWARD);
            if (!lstPreds.isEmpty()) {
                ...
                for (Entry<Integer, Set<Integer>> entry : mapExceptionRanges.entrySet()) {
                    ...
                    if (range.contains(id)) {
                        ...
                        if (!range.contains(childid)) {
                            ...
                            // after replacing 'new Integer(-1)' with '-1' Eclipse throws a
                            // NullPointerException on the following line
                            // could be a bug in Eclipse or some obscure specification glitch,
