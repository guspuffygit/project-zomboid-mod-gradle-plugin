...
public final class IrreducibleCFGDeobfuscator {
    ...
    private static Statement getCandidateForSplitting(Statement statement) {
        ...
        for (Statement stat : statement.getStats()) {
            Set<Statement> setPreds =
                    stat.getNeighboursSet(StatEdge.TYPE_REGULAR, Statement.DIRECTION_BACKWARD);

            if (setPreds.size() > 1) {
                int succCount =
                        stat.getNeighboursSet(StatEdge.TYPE_REGULAR, Statement.DIRECTION_FORWARD)
                                .size();
    ...
    public static boolean splitIrreducibleNode(Statement statement) {
        ...
        for (StatEdge prededge : splitnode.getPredecessorEdges(Statement.STATEDGE_DIRECT_ALL)) {
            if (prededge.getSource() == enteredge.getSource()
                ...
                prededge.getSource()
                        .changeEdgeNode(Statement.DIRECTION_FORWARD, prededge, splitcopy);
    ...
    private static Statement copyStatement(
            Statement from, Statement to, HashMap<Statement, Statement> mapAltToCopies) {
        if (to == null) {
