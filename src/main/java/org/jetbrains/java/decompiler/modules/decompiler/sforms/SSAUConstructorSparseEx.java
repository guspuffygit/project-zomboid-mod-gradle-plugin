// Copyright 2000-2017 JetBrains s.r.o. Use of this source code is governed by the Apache 2.0
...
public class SSAUConstructorSparseEx {
    ...
    // private HashMap<String, HashMap<Integer, FastSet<Integer>>> inVarVersions = new
    // HashMap<String, HashMap<Integer, FastSet<Integer>>>();

    // node id, var, version (direct branch)
    private final HashMap<String, SFormsFastMapDirect> outVarVersions = new HashMap<>();
    // private HashMap<String, HashMap<Integer, FastSet<Integer>>> outVarVersions = new
    // HashMap<String, HashMap<Integer, FastSet<Integer>>>();

    // node id, var, version (negative branch)
    private final HashMap<String, SFormsFastMapDirect> outNegVarVersions = new HashMap<>();
    // private HashMap<String, HashMap<Integer, FastSet<Integer>>> outNegVarVersions = new
    // HashMap<String, HashMap<Integer, FastSet<Integer>>>();

    // node id, var, version
    private final HashMap<String, SFormsFastMapDirect> extraVarVersions = new HashMap<>();
    // private HashMap<String, HashMap<Integer, FastSet<Integer>>> extraVarVersions = new
    // HashMap<String, HashMap<Integer, FastSet<Integer>>>();
    ...
    // version, version
    private final HashMap<VarVersionPair, VarVersionPair> phantomppnodes =
            new HashMap<>(); // ++ and --
    ...
    private void ssaStatements(DirectGraph dgraph, HashSet<String> updated, boolean calcLiveVars) {
        for (DirectNode node : dgraph.nodes) {
            ...
            // quick solution: 'dummy' field variables should not cross basic block borders
            // (otherwise problems e.g. with finally loops - usage without assignment in a loop)
            // For the full solution consider adding a dummy assignment at the entry point of the
            // method
            boolean allow_field_propagation =
                    node.succs.isEmpty()
                            || (node.succs.size() == 1 && node.succs.get(0).preds.size() == 1);
            ...
            boolean this_updated =
                    !mapsEqual(varmaparr[0], outVarVersions.get(node.id))
                            || (outNegVarVersions.containsKey(node.id)
                                    && !mapsEqual(varmaparr[1], outNegVarVersions.get(node.id)));
    ...
    private void processExprent(
            Exprent expr, SFormsFastMapDirect[] varmaparr, Statement stat, boolean calcLiveVars) {
        if (expr == null) {
            return;
        }
            ...
            case Exprent.EXPRENT_FUNCTION:
                ...
                switch (func.getFuncType()) {
                    case FunctionExprent.FUNCTION_IIF:
                        ...
                        SFormsFastMapDirect[] varmaparrNeg =
                                new SFormsFastMapDirect[] {varmapFalse, null};
                        processExprent(
                                func.getLstOperands().get(2), varmaparrNeg, stat, calcLiveVars);
                    ...
                    case FunctionExprent.FUNCTION_CADD:
                        ...
                        SFormsFastMapDirect[] varmaparrAnd =
                                new SFormsFastMapDirect[] {
                                    new SFormsFastMapDirect(varmaparr[0]), null
                                };

                        processExprent(
                                func.getLstOperands().get(1), varmaparrAnd, stat, calcLiveVars);
                    ...
                    case FunctionExprent.FUNCTION_COR:
                        ...
                        processExprent(
                                func.getLstOperands().get(1), varmaparrOr, stat, calcLiveVars);
        ...
        if (varassign != null) {
            ...
            if (varassign.getVersion() == 0) {
                ...
                if (calcLiveVars) {
                    varMapToGraph(
                            new VarVersionPair(varindex.intValue(), varassign.getVersion()),
                            varmap);
                }
            ...
            switch (func.getFuncType()) {
                ...
                case FunctionExprent.FUNCTION_PPI:
                    if (func.getLstOperands().get(0).type == Exprent.EXPRENT_VAR) {
                        ...
                        VarVersionPair varpaar =
                                new VarVersionPair(varindex.intValue(), var.getVersion());
            ...
            if (cardinality == 1) { // size == 1
                if (current_vers != 0) {
                    ...
                    VarVersionNode prenode =
                            ssuversions.nodes.getWithKey(new VarVersionPair(varindex, lastver));
                    VarVersionNode usenode =
                            ssuversions.createNode(new VarVersionPair(varindex, usever));
                    VarVersionEdge edge =
                            new VarVersionEdge(VarVersionEdge.EDGE_GENERAL, prenode, usenode);
    ...
    private void createOrUpdatePhiNode(
            VarVersionPair phivar, FastSparseSet<Integer> vers, Statement stat) {
        ...
        for (Integer ver : versCopy) {
            VarVersionNode prenode =
                    ssuversions.nodes.getWithKey(new VarVersionPair(phivar.var, ver.intValue()));
            ...
            VarVersionEdge edge =
                    new VarVersionEdge(VarVersionEdge.EDGE_GENERAL, prenode, tempnode);
    ...
    private SFormsFastMapDirect getFilteredOutMap(
            String nodeid, String predid, DirectGraph dgraph, String destid) {
        SFormsFastMapDirect mapNew = new SFormsFastMapDirect();
        ...
        if (isFinallyExit) {
            ...
            boolean isExceptionMonitorExit =
                    (exceptionDest != null && !nodeid.equals(exceptionDest));
            ...
            if (isExceptionMonitorExit) {
                ...
                if (!mapTrueSource.isEmpty() && !mapNew.isEmpty()) { // FIXME: what for??
                    ...
                    HashMap<VarVersionPair, VarVersionPair> mapPhantom =
                            phantomexitnodes.get(predid);
                    if (mapPhantom == null) {
                    ...
                    for (Entry<Integer, FastSparseSet<Integer>> ent : mapExitVar.entryList()) {
                        for (Integer version : ent.getValue()) {
                            ...
                            if (phantomvar == null) {
                                ...
                                VarVersionEdge edge =
                                        new VarVersionEdge(
                                                VarVersionEdge.EDGE_PHANTOM, exitnode, phantomnode);
    ...
    private static SFormsFastMapDirect mergeMaps(
            SFormsFastMapDirect mapTo, SFormsFastMapDirect map2) {
    ...
    private void setCatchMaps(
            Statement stat, DirectGraph dgraph, FlattenStatementsHelper flatthelper) {
        SFormsFastMapDirect map;
        ...
        switch (stat.type) {
            ...
            case Statement.TYPE_TRYCATCH:
                ...
                for (int i = 1; i < stat.getStats().size(); i++) {
                    ...
                    extraVarVersions.put(
                            dgraph.nodes.getWithKey(
                                            flatthelper.getMapDestinationNodes()
                                                    .get(stat.getStats().get(i).id)[0])
                                    .id,
                            map);
