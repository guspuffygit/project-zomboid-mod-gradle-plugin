...
public final class IdeaNotNullHelper {
    ...
    public static boolean removeHardcodedChecks(Statement root, StructMethod mt) {
        ...
        while (findAndRemoveParameterCheck(
                root,
        ...
        while (findAndRemoveReturnCheck(
                root, mt)) { // iterate until nothing found. Each invocation handles one method exit
            // check.
            checks_removed = true;
        }
                ...
                if (second_param.type == Exprent.EXPRENT_CONST
                    ...
                    if (first_param.type == Exprent.EXPRENT_VAR) {
                        ...
                        if (param_annotations != null) {
                            List<List<AnnotationExprent>> param_annotations_lists =
                                    param_annotations.getParamAnnotations();
                                        ...
                                        int shift =
                                                method_param_number
                                                        - param_annotations_lists
                                                                .size(); // NOTE: workaround for
                                        // compiler bug, count
                                        // annotations starting
                                        // with the last parameter

                                        List<AnnotationExprent> annotations =
                                                param_annotations_lists.get(i - shift);
    ...
    private static boolean findAndRemoveReturnCheck(Statement stat, StructMethod mt) {
        ...
        StructAnnotationAttribute attr =
                mt.getAttribute(StructGeneralAttribute.ATTRIBUTE_RUNTIME_INVISIBLE_ANNOTATIONS);
        if (attr != null) {
                ...
                if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
                    ...
                    if (ifparent.getElsestat() == stat
                            ...
                            && ((FunctionExprent) if_condition).getFuncType()
                                    == FunctionExprent
                                            .FUNCTION_EQ) { // TODO: reversed order possible (in
                        // theory)
                        ...
                        if (second_param.type == Exprent.EXPRENT_CONST
                                && second_param.getExprType().type
                                        == CodeConstants
                                                .TYPE_NULL) { // TODO: reversed parameter order
                            // if(first_param.type == Exprent.EXPRENT_VAR &&
                            // ((VarExprent)first_param).getIndex() == var_value.getIndex()) {
                            if (first_param.equals(
                                    exprent_value)) { // TODO: check for absence of side effects
                                // like method invocations etc.
                                if (ifbranch.type == Statement.TYPE_BASICBLOCK
                                        && ifbranch.getExprents().size() == 1
                                        &&
                                        // TODO: special check for IllegalStateException
                                        ifbranch.getExprents().get(0).type
                                    ...
                                    if (!ifbranch.getAllSuccessorEdges().isEmpty()) {
                                        ifbranch.removeSuccessor(
                                                ifbranch.getAllSuccessorEdges().get(0));
                                    }

                                    if (!ifparent.getFirst().getExprents().isEmpty()) {
                                        elsebranch
                                                .getExprents()
                                                .addAll(0, ifparent.getFirst().getExprents());
                                    }
            ...
            if (exprent.type == Exprent.EXPRENT_EXIT) {
                ...
                if (exit_exprent.getExitType() == ExitExprent.EXIT_RETURN) {
                    ...
                    if (sequence_stats_number > 1
                            ...
                            && sequence.getStats().get(sequence_stats_number - 2).type
                                    == Statement.TYPE_IF) {
                        IfStatement ifstat =
                                (IfStatement) sequence.getStats().get(sequence_stats_number - 2);
                        Exprent if_condition = ifstat.getHeadexprent().getCondition();
                                ...
                                if (first_param.equals(
                                    ...
                                    if (ifbranch.type == Statement.TYPE_BASICBLOCK
                                        ...
                                        if (!ifstat.getFirst().getExprents().isEmpty()) {
                                            stat.getExprents()
                                                    .addAll(0, ifstat.getFirst().getExprents());
                                        }
