...
public final class CatchAllStatement extends Statement {
    ...
    public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
        ...
        buf.append(isFinally ? " finally" : " catch (" + vars.get(0).toJava(indent, tracer) + ")")
                .append(" {")
                .appendLineSeparator();
        tracer.incrementCurrentSourceLine();
        ...
        if (monitor != null) {
            buf.appendIndent(indent + 1)
                    .append("if (")
                    .append(monitor.toJava(indent, tracer))
                    .append(") {")
                    .appendLineSeparator();
            tracer.incrementCurrentSourceLine();
        }
        ...
        buf.append(
                ExprProcessor.jmpWrapper(
                        handler, indent + 1 + (monitor != null ? 1 : 0), true, tracer));
