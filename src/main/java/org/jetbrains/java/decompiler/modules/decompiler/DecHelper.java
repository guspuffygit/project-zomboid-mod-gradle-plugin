...
public final class DecHelper {
    ...
    public static boolean checkStatementExceptions(List<? extends Statement> lst) {
        ...
        for (Statement stat : lst) {
            Set<Statement> setNew =
                    stat.getNeighboursSet(StatEdge.TYPE_EXCEPTION, Statement.DIRECTION_FORWARD);
    ...
    public static boolean isChoiceStatement(Statement head, List<? super Statement> lst) {
        ...
        Set<Statement> setDest =
                head.getNeighboursSet(StatEdge.TYPE_REGULAR, Statement.DIRECTION_FORWARD);
        ...
        while (true) {
            ...
            for (Statement stat : setDest) {
                ...
                // preds
                Set<Statement> setPred =
                        stat.getNeighboursSet(StatEdge.TYPE_REGULAR, Statement.DIRECTION_BACKWARD);
                setPred.remove(head);
                ...
                if (!setDest.containsAll(setPred) || setPred.size() > 1) {
                    ...
                    while (lst.contains(pred)) {
                        Set<Statement> setPredTemp =
                                pred.getNeighboursSet(
                                        StatEdge.TYPE_REGULAR, Statement.DIRECTION_BACKWARD);
                ...
                if (lstEdges.size() > 1) {
                    Set<Statement> setSucc =
                            stat.getNeighboursSet(
                                    Statement.STATEDGE_DIRECT_ALL, Statement.DIRECTION_FORWARD);
    ...
    public static Set<Statement> getUniquePredExceptions(Statement head) {
        ...
        setHandlers.removeIf(
                statement -> statement.getPredecessorEdges(StatEdge.TYPE_EXCEPTION).size() > 1);
        return setHandlers;
    }
