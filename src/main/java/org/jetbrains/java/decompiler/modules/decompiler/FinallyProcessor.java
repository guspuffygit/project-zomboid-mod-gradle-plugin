...
public class FinallyProcessor {
    ...
    private boolean processStatementEx(
            StructMethod mt, RootStatement root, ControlFlowGraph graph) {
            ...
            if (parent != null
                ...
                if (catchallBlockIDs.containsKey(handler.id)) {
                    ...
                    if (inf == null) { // inconsistent finally
                        ...
                        DeadCodeHelper.removeDeadBlocks(
                                graph); // e.g. multiple return blocks after a nested finally
    ...
    private Record getFinallyInformation(
            StructMethod mt, RootStatement root, CatchAllStatement fstat) {
        ...
        VarVersionPair varpaar =
                new VarVersionPair(
                        (VarExprent)
                                ((AssignmentExprent) lstExprents.get(firstcode == 2 ? 1 : 0))
                                        .getLeft());
        ...
        while (!stack.isEmpty()) {
                    ...
                    if (firstcode == 0) {
                        ...
                        if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
                            ...
                            if (assexpr.getRight().type == Exprent.EXPRENT_VAR
                                    && new VarVersionPair((VarExprent) assexpr.getRight())
                                            .equals(varpaar)) {
                                Exprent next = null;
            ...
            if (blockStatement != null && blockStatement.getBlock() != null) {
                ...
                for (StatEdge edge :
                        blockStatement.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)) {
    ...
    private static void insertBlockBefore(
            ControlFlowGraph graph, BasicBlock oldblock, BasicBlock newblock) {
    ...
    private boolean verifyFinallyEx(
            ControlFlowGraph graph, CatchAllStatement fstat, Record information) {
        ...
        for (BasicBlock start : startBlocks) {
            Area arr =
                    compareSubgraphsEx(
                            graph, start, catchBlocks, first, finallytype, mapLast, skippedFirst);
            if (arr == null) {
                return false;
            }
    ...
    private Area compareSubgraphsEx(
        ...
        class BlockStackEntry {
            ...
            BlockStackEntry(
                    BasicBlock blockCatch, BasicBlock blockSample, List<int[]> lstStoreVars) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (isLastBlock && blockSample.getSeq().isEmpty()) {
                ...
                if (blockCatch.getSuccExceptions().size()
                        == blockSample.getSuccExceptions().size()) {
                    for (int i = 0; i < blockCatch.getSuccExceptions().size(); i++) {
                        ...
                        String excCatch =
                                graph.getExceptionRange(sucCatch, blockCatch)
                                        .getUniqueExceptionsString();
                        String excSample =
                                graph.getExceptionRange(sucSample, blockSample)
                                        .getUniqueExceptionsString();

                        // FIXME: compare handlers if possible
                        boolean equalexc =
                                excCatch == null ? excSample == null : excCatch.equals(excSample);

                        if (equalexc) {
                            if (catchBlocks.contains(sucCatch) && !setSample.contains(sucSample)) {
                                List<int[]> lst = entry.lstStoreVars;

                                if (sucCatch.getSeq().length() > 0
                                        && sucSample.getSeq().length() > 0) {
                                    ...
                                    if (instrCatch.opcode == CodeConstants.opc_astore
                                        ...
                                        lst.add(
                                                new int[] {
                                                    instrCatch.operand(0), instrSample.operand(0)
                                                });
                                    }
                                }
        ...
        return new Area(
                startSample, setSample, getUniqueNext(graph, new HashSet<>(mapNext.values())));
    }
    ...
    public boolean equalInstructions(
            Instruction first, Instruction second, List<int[]> lstStoreVars) {
            ...
            for (int i = 0; i < first.operandsCount(); i++) {
                ...
                if (firstOp != secondOp) {
                    ...
                    if (first.opcode == CodeConstants.opc_aload
                            || first.opcode == CodeConstants.opc_astore) {
    ...
    private static void removeExceptionInstructionsEx(
            BasicBlock block, int blocktype, int finallytype) {
