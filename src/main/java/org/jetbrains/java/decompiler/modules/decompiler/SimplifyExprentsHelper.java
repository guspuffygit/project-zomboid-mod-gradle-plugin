...
public class SimplifyExprentsHelper {
    ...
    private static final MatchEngine class14Builder =
            new MatchEngine(
    ...
    public boolean simplifyStackVarsStatement(
            Statement stat,
        ...
        if (expressions == null) {
            ...
            while (true) {
                ...
                for (Statement st : stat.getStats()) {
                    ...
                    changed =
                            IfHelper.mergeIfs(st, setReorderedIfs)
                                    || // collapse composed if's
                                    buildIff(st, ssa)
                                    || // collapse iff ?: statement
                                    processClass14
                                            && collapseInlinedClass14(
                                                    st); // collapse inlined .class property in
    ...
    private static boolean addArrayInitializer(Exprent first, Exprent second) {
        if (first.type == Exprent.EXPRENT_ASSIGNMENT) {
            ...
            if (as.getRight().type == Exprent.EXPRENT_NEW
                    && as.getLeft().type == Exprent.EXPRENT_VAR) {
                                ...
                                if (constValue < newExpr.getLstArrayElements().size()) {
                                    ...
                                    if (init.type == Exprent.EXPRENT_CONST) {
                                        ...
                                        ConstExprent defaultVal =
                                                ExprProcessor.getDefaultArrayValue(arrType);
                        ...
                        if (expr.type == Exprent.EXPRENT_ASSIGNMENT) {
                            ...
                            if (aas.getLeft().type == Exprent.EXPRENT_ARRAY) {
                                ...
                                if (arrExpr.getArray().type == Exprent.EXPRENT_VAR
                                        && arrVar.equals(arrExpr.getArray())
                                        && arrExpr.getIndex().type == Exprent.EXPRENT_CONST) {
                                    // TODO: check for a number type. Failure extremely improbable,
                                    // but nevertheless...
                                    int constValue =
                                            ((ConstExprent) arrExpr.getIndex()).getIntValue();
    ...
    private static boolean isTrivialStackAssignment(Exprent first) {
        if (first.type == Exprent.EXPRENT_ASSIGNMENT) {
            ...
            if (asf.getLeft().type == Exprent.EXPRENT_VAR
                    && asf.getRight().type == Exprent.EXPRENT_VAR) {
    ...
    private static boolean isStackAssignment2(
            Exprent first, Exprent second) { // e.g. 1.4-style class invocation
        if (first.type == Exprent.EXPRENT_ASSIGNMENT && second.type == Exprent.EXPRENT_ASSIGNMENT) {
            ...
            if (asf.getLeft().type == Exprent.EXPRENT_VAR
                ...
                if (ass.getLeft().type != Exprent.EXPRENT_VAR
                        || !((VarExprent) ass.getLeft()).isStack()) {
                    asf.setRight(
                            new AssignmentExprent(ass.getLeft(), asf.getRight(), ass.bytecode));
                    return true;
                }
            }
        }
    ...
    private static boolean isQualifiedNewGetClass(Exprent first, Exprent second) {
        if (first.type == Exprent.EXPRENT_INVOCATION) {
            ...
            if (!invocation.isStatic()
                    && invocation.getInstance().type == Exprent.EXPRENT_VAR
            ...
            if (as.getLeft().type == Exprent.EXPRENT_VAR
                ...
                if (newType.type == CodeConstants.TYPE_OBJECT
                    ...
                    for (int i = index + 1; i < list.size(); i++) {
                        ...
                        if (setVars.contains(
                                leftPair)) { // variable used somewhere in between -> exit, need a
                            // better reduced code
                            return false;
                        }
                    }
    ...
    private static Exprent isLambda(Exprent exprent, StructClass cl) {
        ...
        if (exprent.type == Exprent.EXPRENT_INVOCATION) {
            ...
            if (in.getInvocationTyp() == InvocationExprent.INVOKE_DYNAMIC) {
                ...
                ClassNode lambda_class =
                        DecompilerContext.getClassProcessor()
                                .getMapRootClasses()
                                .get(lambda_class_name);

                if (lambda_class
                        != null) { // real lambda class found, replace invocation with an anonymous
                    // class
                    NewExprent newExpr =
                            new NewExprent(
                                    new VarType(lambda_class_name, true), null, 0, in.bytecode);
                    newExpr.setConstructor(in);
                    // note: we don't set the instance to null with in.setInstance(null) like it is
                    // done for a common constructor invocation
                    // lambda can also be a reference to a virtual method (e.g. String x;
                    // ...(x::toString);)
        ...
        if (stat.type == Statement.TYPE_IF && stat.getExprents() == null) {
            ...
            if (statement.iftype == IfStatement.IFTYPE_IFELSE) {
                ...
                if (ifStatement.getExprents() != null
                        && ifStatement.getExprents().size() == 1
                        && elseStatement.getExprents() != null
                        && elseStatement.getExprents().size() == 1
                        && ifStatement.getAllSuccessorEdges().size() == 1
                        && elseStatement.getAllSuccessorEdges().size() == 1
                        && ifStatement.getAllSuccessorEdges().get(0).getDestination()
                                == elseStatement.getAllSuccessorEdges().get(0).getDestination()) {
                    ...
                    if (ifExpr.type == Exprent.EXPRENT_ASSIGNMENT
                        ...
                        if (ifAssign.getLeft().type == Exprent.EXPRENT_VAR
                                && elseAssign.getLeft().type == Exprent.EXPRENT_VAR) {
                            ...
                            if (ifVar.getIndex() == elseVar.getIndex()
                                    && ifVar.isStack()) { // ifVar.getIndex() >=
                                // VarExprent.STACK_BASE) {
                                boolean found = false;

                                for (Entry<VarVersionPair, FastSparseSet<Integer>> ent :
                                        ssa.getPhi().entrySet()) {
                                    if (ent.getKey().var == ifVar.getIndex()) {
                                        if (ent.getValue().contains(ifVar.getVersion())
                                                && ent.getValue().contains(elseVar.getVersion())) {
                                            found = true;
                                            break;
                                        }
                                    }
                                ...
                                if (found) {
                                    List<Exprent> data =
                                            new ArrayList<>(statement.getFirst().getExprents());
                    ...
                    } else if (ifExpr.type == Exprent.EXPRENT_EXIT
                            && elseExpr.type == Exprent.EXPRENT_EXIT) {
                        ...
                        if (ifExit.getExitType() == elseExit.getExitType()
                            ...
                            // throw is dangerous, because of implicit casting to a common
                            // superclass
                            // e.g. throws IOException and throw true?new RuntimeException():new
                            // IOException(); won't work
                            if (ifExit.getExitType() == ExitExprent.EXIT_THROW
                                    && !ifExit.getValue()
                                            .getExprType()
                                            .equals(
                                                    elseExit.getValue()
                                                            .getExprType())) { // note: getExprType
                                // unreliable at this
                                // point!
                                return false;
                            }
                            ...
                            Statement closure =
                                    retEdge.closure == statement
                                            ? statement.getParent()
                                            : retEdge.closure;
                            statement.addSuccessor(
                                    new StatEdge(
                                            StatEdge.TYPE_BREAK,
                                            statement,
    ...
    private static boolean collapseInlinedClass14(Statement stat) {
        ...
        if (ret) {
            ...
            AssignmentExprent assignment =
                    (AssignmentExprent) class14Builder.getVariableValue("$assignfield$");
            FieldExprent fieldExpr = (FieldExprent) class14Builder.getVariableValue("$field$");

            assignment.replaceExprent(
                    assignment.getRight(),
            ...
            ClassWrapper wrapper =
                    (ClassWrapper)
                            DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS_WRAPPER);
            if (wrapper != null) {
