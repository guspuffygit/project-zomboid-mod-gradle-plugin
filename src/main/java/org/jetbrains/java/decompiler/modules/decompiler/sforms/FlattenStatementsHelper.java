...
public class FlattenStatementsHelper {
    ...
    public DirectGraph buildDirectGraph(RootStatement root) {
        ...
        DirectNode node =
                new DirectNode(DirectNode.NODE_DIRECT, dummyexit, dummyexit.id.toString());
            ...
            if (statEntry.succEdges == null) {
                switch (stat.type) {
                    case Statement.TYPE_BASICBLOCK:
                        node =
                                new DirectNode(
                                        DirectNode.NODE_DIRECT, stat, (BasicBlockStatement) stat);
                        ...
                        if (tailExprentList != null) {
                            DirectNode tail =
                                    new DirectNode(DirectNode.NODE_TAIL, stat, stat.id + "_tail");
                        ...
                        if (stat.getLastBasicType() == Statement.LASTBASICTYPE_IF) {
                            mapPosIfBranch.put(
                                    sourcenode.id, lstSuccEdges.get(0).getDestination().id);
                        }
                    ...
                    case Statement.TYPE_TRYCATCH:
                        DirectNode firstnd =
                                new DirectNode(DirectNode.NODE_TRY, stat, stat.id + "_try");
                        ...
                        for (Statement st : stat.getStats()) {
                            ...
                            if (stat.type == Statement.TYPE_CATCHALL
                                    && ((CatchAllStatement) stat).isFinally()) {
                                stack = new LinkedList<>(stackFinally);

                                if (st == stat.getFirst()) { // catch head
                                    stack.add(
                                            new StackEntry(
                                                    (CatchAllStatement) stat, Boolean.FALSE));
                                } else { // handler
                                    stack.add(
                                            new StackEntry(
                                                    (CatchAllStatement) stat,
                                                    Boolean.TRUE,
                                                    StatEdge.TYPE_BREAK,
                                                    root.getDummyExit(),
                                                    st,
                    ...
                    case Statement.TYPE_DO:
                        if (statementBreakIndex == 0) {
                            ...
                            lstStackStatements.addFirst(
                                    new StatementStackEntry(stat.getFirst(), stackFinally, null));
                        ...
                        lstSuccEdges.add(
                                stat.getSuccessorEdges(Statement.STATEDGE_DIRECT_ALL)
                        ...
                        switch (looptype) {
                            ...
                            case DoStatement.LOOP_DOWHILE:
                                node =
                                        new DirectNode(
                                                DirectNode.NODE_CONDITION, stat, stat.id + "_cond");
                                ...
                                if (looptype == DoStatement.LOOP_WHILE) {
                                    mapDestinationNodes.put(
                                            stat.id, new String[] {node.id, node.id});
                                } else {
                                    mapDestinationNodes.put(stat.id, new String[] {nd.id, node.id});

                                    boolean found = false;
                                    for (Edge edge : listEdges) {
                                        if (edge.statid.equals(stat.id)
                                                && edge.edgetype == StatEdge.TYPE_CONTINUE) {
                                            found = true;
                                            break;
                                        }
                                    }
                                    if (!found) {
                                        listEdges.add(
                                                new Edge(nd.id, stat.id, StatEdge.TYPE_CONTINUE));
                                    }
                                }
                                sourcenode = node;
                                break;
                            case DoStatement.LOOP_FOR:
                                DirectNode nodeinit =
                                        new DirectNode(
                                                DirectNode.NODE_INIT, stat, stat.id + "_init");
                                ...
                                DirectNode nodeinc =
                                        new DirectNode(
                                                DirectNode.NODE_INCREMENT, stat, stat.id + "_inc");
                                nodeinc.exprents = dostat.getIncExprentList();
                                graph.nodes.putWithKey(nodeinc, nodeinc.id);

                                mapDestinationNodes.put(
                                        stat.id, new String[] {nodeinit.id, nodeinc.id});
                                ...
                                listEdges.add(
                                        new Edge(
                                                ...
                                                StatEdge.TYPE_REGULAR));
                                listEdges.add(
                                        new Edge(nodeinit.id, -stat.id, StatEdge.TYPE_REGULAR));
                                listEdges.add(
                                        new Edge(nodeinc.id, -stat.id, StatEdge.TYPE_REGULAR));
                    ...
                    case Statement.TYPE_ROOT:
                        ...
                        if (statementBreakIndex <= statsize) {
                            ...
                            switch (stat.type) {
                                case Statement.TYPE_SYNCRONIZED:
                                    tailexprlst =
                                            ((SynchronizedStatement) stat).getHeadexprentList();
                                    break;
                                case Statement.TYPE_SWITCH:
                            ...
                            node =
                                    graph.nodes.getWithKey(
                                            mapDestinationNodes.get(stat.getFirst().id)[0]);
                            mapDestinationNodes.put(stat.id, new String[] {node.id, null});
                        ...
                        if (entry == null) {
                            ...
                            if (entry.state) { // finally handler statement
                                if (edgetype == StatEdge.TYPE_FINALLYEXIT) {
                                    ...
                                    isFinallyExit = true;
                                    isFinallyMonitorExceptionPath =
                                            (catchall.getMonitor() != null)
                                                    & entry.isFinallyExceptionPath;
                                ...
                                if (!catchall.containsStatementStrict(destination)) {
                                    ...
                                    lstStackStatements.addFirst(
                                            new StatementStackEntry(
                                                    catchall.getHandler(), stack, null));
    ...
    private void saveEdge(
        ...
        if (finallyShortRangeSource != null) {
            ...
            mapShortRangeFinallyPathIds
                    .computeIfAbsent(sourcenode.id, k -> new ArrayList<>())
                    .add(
                            new String[] {
    ...
    private void setEdges() {
        for (Edge edge : listEdges) {
            ...
            DirectNode dest =
                    graph.nodes.getWithKey(
                            mapDestinationNodes
                                    .get(statid)[edge.edgetype == StatEdge.TYPE_CONTINUE ? 1 : 0]);
            ...
            if (mapPosIfBranch.containsKey(sourceid)
                    && !statid.equals(mapPosIfBranch.get(sourceid))) {
                graph.mapNegIfBranch.put(sourceid, dest.id);
            }
        }

        for (int i = 0; i < 2; i++) {
            for (Entry<String, List<String[]>> ent :
                    (i == 0 ? mapShortRangeFinallyPathIds : mapLongRangeFinallyPathIds)
                            .entrySet()) {
                ...
                for (String[] arr : lst) {
                    ...
                    DirectNode dest =
                            graph.nodes.getWithKey(
                                    mapDestinationNodes
                                            .get(Integer.parseInt(arr[1]))[isContinueEdge ? 1 : 0]);
                    DirectNode enter =
                            graph.nodes.getWithKey(
                                    mapDestinationNodes.get(Integer.parseInt(arr[2]))[0]);
