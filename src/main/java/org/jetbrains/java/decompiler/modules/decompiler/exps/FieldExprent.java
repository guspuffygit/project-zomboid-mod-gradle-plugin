...
public class FieldExprent extends Exprent {
    ...
    public int getExprentUse() {
        return 0; // multiple references to a field considered dangerous in a multithreaded
        // environment, thus no Exprent.MULTIPLE_USES set here
    }
    ...
    @Override
    public Exprent copy() {
        return new FieldExprent(
                name,
                classname,
                isStatic,
                instance == null ? null : instance.copy(),
                descriptor,
                bytecode);
    }
    ...
    public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
        ...
        if (isStatic) {
            ...
            if (node == null
                    || !classname.equals(node.classStruct.qualifiedName)
                    || isAmbiguous()) {
                buf.append(
            ...
            if (buf.toString()
                    .equals(VarExprent.VAR_NAMELESS_ENCLOSURE)) { // FIXME: workaround for field
                // access of an anonymous enclosing
                // class. Find a better way.
                buf.setLength(0);
            } else {
                buf.append(".");
            }
