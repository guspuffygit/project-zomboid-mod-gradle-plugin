...
public final class IfHelper {
                ...
                if (ifbranch.succs.get(1).value == rtnode.succs.get(1).value) {
                    ...
                    if (ifchild.getFirst().getExprents().isEmpty()) {
                        ...
                        if (ifbranch.edgetypes.get(0) == 1) { // target null
                            ...
                            StatEdge ifedge =
                                    new StatEdge(
                                            StatEdge.TYPE_REGULAR, ifparent.getFirst(), ifinner);
                            ifparent.getFirst().addSuccessor(ifedge);
                            ifparent.setIfEdge(ifedge);
                    ...
                    if (ifchild.getFirst().getExprents().isEmpty()) {
                        ...
                        lstOperands.add(
                                new FunctionExprent(
                                        ...
                                        ifchild.getHeadexprent().getCondition(),
                                        null));
                        statexpr.setCondition(
                                new FunctionExprent(
                                        FunctionExprent.FUNCTION_CADD, lstOperands, null));
    ...
    private static boolean collapseElse(IfNode rtnode) {
        if (rtnode.edgetypes.get(1) == 0) {
            ...
            if (elsebranch.succs.size() == 2) {
                ...
                int path =
                        elsebranch.succs.get(1).value == rtnode.succs.get(0).value
                                ? 2
                    ...
                    if (secondif.getFirst().getExprents().isEmpty()) {
                        ...
                        for (StatEdge edge : firstif.getAllPredecessorEdges()) {
                            if (!firstif.containsStatementStrict(edge.getSource())) {
                                ...
                                edge.getSource()
                                        .changeEdgeNode(
                                                Statement.DIRECTION_FORWARD, edge, secondif);
                ...
                if (elsebranch.succs.get(0).value == rtnode.succs.get(0).value) {
                    ...
                    StatEdge newifedge =
                            new StatEdge(StatEdge.TYPE_REGULAR, firstif.getFirst(), second);
    ...
    private static IfNode buildGraph(IfStatement stat, boolean stsingle) {
        ...
        if (ifchild == null) {
            ...
            if (!ifchild.getAllSuccessorEdges().isEmpty()) {
                ifnode.addChild(
                        new IfNode(ifchild.getAllSuccessorEdges().get(0).getDestination()), 1);
            }
        }
        ...
        if (stsingle || edge.getType() != StatEdge.TYPE_REGULAR) {
            ...
            if (!elsechild.getAllSuccessorEdges().isEmpty()) {
                elsenode.addChild(
                        new IfNode(elsechild.getAllSuccessorEdges().get(0).getDestination()), 1);
            }
        }
    ...
    private static boolean reorderIf(IfStatement ifstat) {
        ...
        if (ifstat.getIfstat() == null) {
            ...
            ifdirect =
                    ifstat.getIfEdge().getType() == StatEdge.TYPE_FINALLYEXIT
            ...
            ifdirect =
                    !lstSuccs.isEmpty() && lstSuccs.get(0).getType() == StatEdge.TYPE_FINALLYEXIT
                            || hasDirectEndEdge(ifstat.getIfstat(), from);
        }

        Statement last =
                parent.type == Statement.TYPE_SEQUENCE ? parent.getStats().getLast() : ifstat;
        noelsestat = (last == ifstat);

        elsedirect =
                !last.getAllSuccessorEdges().isEmpty()
                                && last.getAllSuccessorEdges().get(0).getType()
                                        == StatEdge.TYPE_FINALLYEXIT
                        || hasDirectEndEdge(last, from);

        if (!noelsestat
                && existsPath(ifstat, ifstat.getAllSuccessorEdges().get(0).getDestination())) {
            return false;
        }
        ...
        if ((ifdirect || ifdirectpath)
            ...
            statexpr.setCondition(
                    new FunctionExprent(
                            FunctionExprent.FUNCTION_BOOL_NOT, statexpr.getCondition(), null));
            ...
            if (noelsestat) {
                ...
                if (noifstat) {
                    ...
                    SequenceStatement newseq =
                            new SequenceStatement(Arrays.asList(ifstat, ifbranch));
