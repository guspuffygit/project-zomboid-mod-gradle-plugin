...
public class VarVersionsProcessor {
    ...
    private static void eliminateNonJavaTypes(VarTypeProcessor typeProcessor) {
        ...
        for (VarVersionPair paar : new ArrayList<>(mapExprentMinTypes.keySet())) {
            ...
            if (type.type == CodeConstants.TYPE_BYTECHAR
                    || type.type == CodeConstants.TYPE_SHORTCHAR) {
                if (maxType != null && maxType.type == CodeConstants.TYPE_CHAR) {
                    type = VarType.VARTYPE_CHAR;
                } else {
                    type =
                            type.type == CodeConstants.TYPE_BYTECHAR
                                    ? VarType.VARTYPE_BYTE
                                    : VarType.VARTYPE_SHORT;
                }
                mapExprentMinTypes.put(paar, type);
                // } else if(type.type == CodeConstants.TYPE_CHAR && (maxType == null ||
                // maxType.type == CodeConstants.TYPE_INT)) { // when possible, lift char to int
    ...
    private static void simpleMerge(
            VarTypeProcessor typeProcessor, DirectGraph graph, StructMethod mt) {
        Map<VarVersionPair, VarType> mapExprentMaxTypes = typeProcessor.getMapExprentMaxTypes();
        Map<VarVersionPair, VarType> mapExprentMinTypes = typeProcessor.getMapExprentMinTypes();
            ...
            if (ent.getValue().size() > 1) {
                ...
                for (int i = 0; i < lstVersions.size(); i++) {
                    ...
                    for (int j = i + 1; j < lstVersions.size(); j++) {
                        VarVersionPair secondPair =
                                new VarVersionPair(ent.getKey(), lstVersions.get(j));
                        ...
                        if (firstType.equals(secondType)
                            ...
                            VarType type =
                                    firstMaxType == null
                                            ...
                                            : secondMaxType == null
                                                    ? firstMaxType
                                                    : VarType.getCommonMinType(
                                                            firstMaxType, secondMaxType);
                            ...
                            typeProcessor
                                    .getMapFinalVars()
                                    .put(firstPair, VarTypeProcessor.VAR_NON_FINAL);
    ...
    private void setNewVarIndices(
        ...
        final Map<VarVersionPair, VarType> mapExprentMaxTypes =
                typeProcessor.getMapExprentMaxTypes();
        Map<VarVersionPair, VarType> mapExprentMinTypes = typeProcessor.getMapExprentMinTypes();
                ...
                exprent -> {
                    ...
                    for (Exprent expr : lst) {
                        if (expr.type == Exprent.EXPRENT_VAR) {
                            ...
                            VarType maxType =
                                    mapExprentMaxTypes.get(new VarVersionPair(expr.id, -1));
