            ...
            if (setHandlers.contains(stat)) {
                ...
                vars.add(
                        new VarExprent(
                                ...
                                new VarType(
                                        CodeConstants.TYPE_OBJECT, 0, edge.getExceptions().get(0)),
                                // FIXME: for now simply the first type. Should get the first common
                                // superclass when possible.
                ...
                if (edge.getExceptions() != null && setHandlers.contains(stat)) {
                    if (stat.getLastBasicType() != LASTBASICTYPE_GENERAL) {
                        ...
                        if (!lstStatSuccs.isEmpty()
                                && lstStatSuccs.get(0).getType() == StatEdge.TYPE_REGULAR) {
    ...
    public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
        ...
        for (int i = 1; i < stats.size(); i++) {
            ...
            if (!block.getSeq().isEmpty()
                    && block.getInstruction(0).opcode == CodeConstants.opc_astore) {
            ...
            if (exception_types.size() > 1) { // multi-catch, Java 7 style
                for (int exc_index = 1; exc_index < exception_types.size(); ++exc_index) {
                    VarType exc_type =
                            new VarType(
                                    CodeConstants.TYPE_OBJECT, 0, exception_types.get(exc_index));
