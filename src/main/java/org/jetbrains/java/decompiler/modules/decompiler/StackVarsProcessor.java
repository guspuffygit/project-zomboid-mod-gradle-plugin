...
public class StackVarsProcessor {
    ...
    private boolean iterateStatements(RootStatement root, SSAUConstructorSparseEx ssa) {
        ...
        while (!stack.isEmpty()) {
            ...
            // make sure the 3 special exprent lists in a loop (init, condition, increment) are not
            // empty
            // change loop type if necessary
            if (nd.exprents.isEmpty()
                    && (nd.type == DirectNode.NODE_INIT
    ...
    private static Exprent isReplaceableVar(
            Exprent exprent, Map<VarVersionPair, Exprent> mapVarValues) {
        Exprent dest = null;
    ...
    private static void replaceSingleVar(
            Exprent parent, VarExprent var, Exprent dest, SSAUConstructorSparseEx ssau) {
        ...
        for (VarVersionPair varpaar : setVars) {
            ...
            for (Iterator<Entry<Integer, FastSparseSet<Integer>>> itent =
                            node.live.entryList().iterator();
                    itent.hasNext(); ) {
    ...
    private int[] iterateExprent(
        ...
        if (!notdom && usedVers.isEmpty()) {
            if (left.isStack()
                    && (right.type == Exprent.EXPRENT_INVOCATION
        ...
        if (isSelfReference && vernotreplaced) {
            ...
            for (VarVersionPair usedver : setTempUsedVers) {
                ...
                if (right.type == Exprent.EXPRENT_FIELD
                        && ssau.getMapFieldVars().containsKey(right.id)) {
    ...
    private static Object[] iterateChildExprent(
            Exprent exprent,
            Exprent parent,
            Exprent next,
            Map<VarVersionPair, Exprent> mapVarValues,
        ...
        if (next == null && parent.type == Exprent.EXPRENT_MONITOR) {
            ...
            if (monexpr.getMonType() == MonitorExprent.MONITOR_ENTER
    ...
    private static boolean getUsedVersions(
            SSAUConstructorSparseEx ssa, VarVersionPair var, List<? super VarVersionNode> res) {
    ...
    private static boolean isVersionToBeReplaced(
        ...
        if (!InterpreterUtil.equalObjects(
                ssau.getMapVersionFirstRange().get(leftpaar),
        ...
        for (Entry<Integer, Set<VarVersionPair>> ent : mapVars.entrySet()) {
            ...
            for (Integer livever : liveverset) {
                VarVersionNode node =
                        ssuversions.nodes.getWithKey(
                                new VarVersionPair(ent.getKey().intValue(), livever.intValue()));
    ...
    private static Map<Integer, Set<VarVersionPair>> getAllVarVersions(
            VarVersionPair leftvar, Exprent exprent, SSAUConstructorSparseEx ssau) {
