        ...
        if (isEnumArray(value)) {
            ...
            if (classNode != null) {
                MethodWrapper wrapper =
                        classNode.getWrapper().getMethodWrapper(CodeConstants.CLINIT_NAME, "()V");
            ...
            switchExprent.replaceExprent(
                    value, ((InvocationExprent) array.getIndex()).getInstance().copy());
        }
    }
        ...
        if (exprent instanceof ArrayExprent) {
            ...
            return field instanceof FieldExprent
                    && (((FieldExprent) field).getName().startsWith("$SwitchMap")
                            || (index instanceof InvocationExprent
                                    && ((InvocationExprent) index).getName().equals("ordinal")));
        }
        return false;
    }
}
