                    ...
                    if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
                        ...
                        if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD
                            ...
                            if (fExpr.getClassname()
                                ...
                                if (structField != null
                                        && structField.hasModifier(CodeConstants.ACC_FINAL)) {
                                    action = 1;
                                }
                            }
                        }
                            ...
                            && Statements.isInvocationInitConstructor(
                ...
                if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
                    ...
                    if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD) {
                        ...
                        if (fExpr.isStatic()
                            ...
                            if (inlineInitializers
                                    || isExprentIndependent(assignExpr.getRight(), method)) {
                                String keyField =
                                ...
                                if (!wrapper.getStaticFieldInitializers().containsKey(keyField)) {
                                    wrapper.getStaticFieldInitializers()
                                            .addWithKey(assignExpr.getRight(), keyField);
                                    firstData.getExprents().remove(0);
                                    found = true;
                                }
                            }
                    ...
                    if (assignExpr.getLeft().type == Exprent.EXPRENT_FIELD) {
                        ...
                        if (!fExpr.isStatic()
                                ...
                                && cl.hasField(
                                        fExpr.getName(),
                                        fExpr.getDescriptor()
                                                .descriptorString)) { // check for the physical
                            // existence of the field.
                            // Could be defined in a
                            // superclass.

                            if (isExprentIndependent(
                                    assignExpr.getRight(), lstMethodWrappers.get(i))) {
                                String fieldKey =
                                        InterpreterUtil.makeUniqueKey(
                                                fExpr.getName(),
                                                fExpr.getDescriptor().descriptorString);
                ...
                case Exprent.EXPRENT_VAR:
                    ...
                    if (!method.varproc.getExternalVars().contains(varPair)) {
                        ...
                        if (!varName.equals("this")
                                && !varName.endsWith(
                                        ".this")) { // FIXME: remove direct comparison with strings
                            return false;
                        }
                    }
                    break;
