...
public class NestedClassProcessor {
            ...
            if (child.type == ClassNode.CLASS_LAMBDA) {
                ...
                if (child.type == ClassNode.CLASS_LOCAL && child.enclosingMethod != null) {
                    MethodWrapper enclosingMethodWrapper =
                            node.getWrapper().getMethods().getWithKey(child.enclosingMethod);
                    if (enclosingMethodWrapper
                            != null) { // e.g. in case of switch-on-enum. FIXME: some proper
                        // handling of multiple enclosing classes
    ...
    private static void setLambdaVars(ClassNode parent, ClassNode child) {
        if (child.lambdaInformation
                .is_method_reference) { // method reference, no code and no parameters
            return;
        }
        ...
        MethodWrapper enclosingMethod =
                parent.getWrapper().getMethods().getWithKey(child.enclosingMethod);
        ...
        MethodDescriptor md_content =
                MethodDescriptor.parseDescriptor(child.lambdaInformation.content_method_descriptor);
                        ...
                        exprent -> {
                            ...
                            for (Exprent expr : lst) {
                                if (expr.type == Exprent.EXPRENT_NEW) {
                                    ...
                                    VarNamesCollector enclosingCollector =
                                            new VarNamesCollector(
                                                    enclosingMethod.varproc.getVarNames());

                                    if (new_expr.isLambda()
                                            && lambda_class_type.equals(new_expr.getNewType())) {
                                        ...
                                        for (int i = 0; i < md_content.params.length; ++i) {
                                            VarVersionPair varVersion =
                                                    new VarVersionPair(varIndex, 0);
                                            if (i < vars_count) {
                                                Exprent param =
                                                        inv_dynamic
                                                                .getLstParameters()
                                                ...
                                                if (param.type == Exprent.EXPRENT_VAR) {
                                                    mapNewNames.put(
                                                            varVersion,
                                                            enclosingMethod.varproc.getVarName(
                                                                    new VarVersionPair(
                                                                            (VarExprent) param)));
                                                }
                                            } else {
                                                mapNewNames.put(
                                                        varVersion,
                                                        enclosingCollector.getFreeName(
                                                                method.varproc.getVarName(
                                                                        varVersion)));
                                            }
            ...
            if ((child.type == ClassNode.CLASS_LOCAL || child.type == ClassNode.CLASS_ANONYMOUS)
                ...
                if (!setEnclosing.isEmpty()) {
                    StructEnclosingMethodAttribute attr =
                            child.classStruct.getAttribute(
                                    StructGeneralAttribute.ATTRIBUTE_ENCLOSING_METHOD);
                    if (attr != null
                            && attr.getMethodName() != null
                            && node.classStruct.qualifiedName.equals(attr.getClassName())
                            && node.classStruct.getMethod(
                                            attr.getMethodName(), attr.getMethodDescriptor())
                                    != null) {
                ...
                if (!hasEnclosing) {
                    if (child.type == ClassNode.CLASS_ANONYMOUS) {
                        ...
                        String message =
                                "Unreferenced local class " + child.classStruct.qualifiedName + "!";
                        DecompilerContext.getLogger()
                                .writeMessage(message, IFernflowerLogger.Severity.WARN);
                    }
                }
    ...
    private static void computeLocalVarsAndDefinitions(ClassNode node) {
        ...
        for (ClassNode nd : node.nested) {
            if (nd.type != ClassNode.CLASS_LAMBDA
                ...
                if (mask.isEmpty()) {
                    String message =
                            "Nested class " + nd.classStruct.qualifiedName + " has no constructor!";
                    DecompilerContext.getLogger()
                            .writeMessage(message, IFernflowerLogger.Severity.WARN);
                } else {
                                        ...
                                        for (Exprent expr : lst) {
                                            if (expr.type == Exprent.EXPRENT_NEW) {
                                                InvocationExprent constructor =
                                                        ((NewExprent) expr).getConstructor();

                                                if (constructor != null
                                                        && mapVarMasks.containsKey(
                                                                constructor
                                                                        .getClassname())) { // non-static inner class constructor
                                                    String refClassName =
                                                            constructor.getClassname();
                                                    ClassNode nestedClassNode =
                                                            node.getClassNode(refClassName);

                                                    if (nestedClassNode.type
                                                            != ClassNode.CLASS_MEMBER) {
                                                        List<VarFieldPair> mask =
                                                                mapVarMasks
                                                                        .get(refClassName)
                                                                        .get(
                                                                                constructor
                                                        ...
                                                        if (!mapVarFieldPairs.containsKey(
                                                                refClassName)) {
                                                            mapVarFieldPairs.put(
                                                                    refClassName, new HashMap<>());
                                                        }

                                                        List<VarFieldPair> lstTemp =
                                                                new ArrayList<>();
                                                        ...
                                                        for (int i = 0; i < mask.size(); i++) {
                                                            ...
                                                            if (param.type == Exprent.EXPRENT_VAR
                                                                ...
                                                                // FIXME: flags of variables are
                                                                // wrong! Correct the entire
                                                                // functionality.
                                                        ...
                                                        if (pairMask == null) {
                                                            ...
                                                            for (int i = 0;
                                                                    i < pairMask.size();
                                                                    i++) {
                                                                if (!InterpreterUtil.equalObjects(
                                                                        pairMask.get(i),
                                                                        lstTemp.get(i))) {
                                                                    pairMask.set(i, null);
                                                                }
        ...
        for (MethodWrapper method : child.getWrapper().getMethods()) {
            if (method.root != null) { // neither abstract nor native
                ...
                if (method.synthParameters != null) {
                    ...
                    MethodDescriptor md =
                            MethodDescriptor.parseDescriptor(method.methodStruct.getDescriptor());

                    for (VarVersionPair pair : method.synthParameters) {
                        if (pair != null) {
                            VarVersionPair newVar =
                                    new VarVersionPair(
                                            method.counter.getCounterAndIncrement(
                                                    CounterContainer.VAR_COUNTER),
                                            0);
                            ...
                            if (child.type != ClassNode.CLASS_MEMBER) {
                                ...
                                enclosingMethod.varproc.setVarFinal(
                                        pair, VarTypeProcessor.VAR_EXPLICIT_FINAL);
                            }
                            ...
                            if (pair.var == -1 || "this".equals(varName)) {
                                ...
                                method.varproc
                                        .getThisVars()
                                        .put(newVar, parent.classStruct.qualifiedName);
                            }
                ...
                for (ClassNode classNode = child; classNode != null; classNode = classNode.parent) {
                    for (Entry<String, VarVersionPair> entry :
                            classNode.mapFieldsToVars.entrySet()) {
                        VarVersionPair newVar =
                                new VarVersionPair(
                                        method.counter.getCounterAndIncrement(
                                                CounterContainer.VAR_COUNTER),
                        ...
                        mapFieldsToNewVars.put(
                                InterpreterUtil.makeUniqueKey(
                                        classNode.classStruct.qualifiedName, entry.getKey()),
                                newVar);
                        ...
                        if (classNode.type != ClassNode.CLASS_MEMBER) {
                            MethodWrapper enclosing_method =
                                    classNode
                                            .parent
                                            .getWrapper()
                                            .getMethods()
                            ...
                            enclosing_method.varproc.setVarFinal(
                                    entry.getValue(), VarTypeProcessor.VAR_EXPLICIT_FINAL);
                        }
                        ...
                        if (entry.getValue().var == -1 || "this".equals(varName)) {
                            ...
                            method.varproc
                                    .getThisVars()
                                    .put(newVar, classNode.parent.classStruct.qualifiedName);
                        }
                        ...
                        if (classNode
                                == child) { // fields higher up the chain were already handled with
                            // their classes
                            StructField fd =
                                    child.classStruct.getFields().getWithKey(entry.getKey());
                            child.getWrapper()
                                    .getHiddenMembers()
                                    .add(
                                            InterpreterUtil.makeUniqueKey(
                                ...
                                new DirectGraph.ExprentIterator() {
                                    ...
                                    public int processExprent(Exprent exprent) {
                                        if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
                                            ...
                                            if (assignExpr.getLeft().type
                                                    == Exprent.EXPRENT_FIELD) {
                                                FieldExprent fExpr =
                                                        (FieldExprent) assignExpr.getLeft();
                                                String qName = child.classStruct.qualifiedName;
                                                if (fExpr.getClassname().equals(qName)
                                                        && // process this class only
                                                        mapFieldsToNewVars.containsKey(
                                                                InterpreterUtil.makeUniqueKey(
                                                                        qName,
                                                                        fExpr.getName(),
                                                                        fExpr.getDescriptor()
                                        ...
                                        if (child.type == ClassNode.CLASS_ANONYMOUS
                                                && CodeConstants.INIT_NAME.equals(
                                                        method.methodStruct.getName())
                                                && exprent.type == Exprent.EXPRENT_INVOCATION) {
                                            InvocationExprent invokeExpr =
                                                    (InvocationExprent) exprent;
                                            if (invokeExpr.getFunctype()
                                                    == InvocationExprent.TYP_INIT) {
                                                // invocation of the super constructor in an
                                                // anonymous class
                                                child.superInvocation =
                                                        invokeExpr; // FIXME: save original names of
                                                // parameters
                                                return 2;
                                            }
                                        }
                                    ...
                                    private Exprent replaceExprent(Exprent exprent) {
                                        if (exprent.type == Exprent.EXPRENT_VAR) {
                                            ...
                                            if (mapParamsToNewVars.containsKey(varIndex)) {
                                                VarVersionPair newVar =
                                                        mapParamsToNewVars.get(varIndex);
    ...
    private static Map<String, List<VarFieldPair>> getMaskLocalVars(ClassWrapper wrapper) {
        ...
        for (StructMethod mt : cl.getMethods()) {
            if (CodeConstants.INIT_NAME.equals(mt.getName())) {
                ...
                MethodWrapper method =
                        wrapper.getMethodWrapper(CodeConstants.INIT_NAME, mt.getDescriptor());
                    ...
                    for (int i = 0; i < md.params.length; i++) { // no static methods allowed
                        ...
                        fields.add(
                                keyField == null
                                        ...
                                        : new VarFieldPair(
                                                keyField,
                                                new VarVersionPair(-1, 0))); // TODO: null?
    ...
    private static String getEnclosingVarField(
            StructClass cl, MethodWrapper method, DirectGraph graph, int index) {
        String field = "";
            ...
            for (Exprent exprent : firstNode.exprents) {
                if (exprent.type == Exprent.EXPRENT_ASSIGNMENT) {
                    ...
                    if (assignExpr.getRight().type == Exprent.EXPRENT_VAR
                        ...
                        StructField fd =
                                cl.getField(left.getName(), left.getDescriptor().descriptorString);
                        if (fd != null
                            ...
                            field =
                                    InterpreterUtil.makeUniqueKey(
                                            left.getName(), left.getDescriptor().descriptorString);
                            break;
                        }
                    }
                }
    ...
    private static boolean possiblySyntheticField(StructField fd) {
        return fd.getName().contains("$")
                && fd.hasModifier(CodeConstants.ACC_FINAL)
                && fd.hasModifier(CodeConstants.ACC_PRIVATE);
    }

    private static void mergeListSignatures(
            List<VarFieldPair> first, List<VarFieldPair> second, boolean both) {
        int i = 1;
    ...
    private static void setLocalClassDefinition(MethodWrapper method, ClassNode node) {
        ...
        VarExprent var =
                new VarExprent(
                        method.counter.getCounterAndIncrement(CounterContainer.VAR_COUNTER),
                        classType,
                        method.varproc);
    ...
    private static Statement getDefStatement(
            Statement stat, VarType classType, Set<? super Statement> setStats) {
        ...
        for (Exprent expr : lst) {
            ...
            switch (expr.type) {
                case Exprent.EXPRENT_CONST:
                    ...
                    res =
                            (VarType.VARTYPE_CLASS.equals(constExpr.getConstType())
                                            && classname.equals(constExpr.getValue())
                ...
                case Exprent.EXPRENT_NEW:
                    ...
                    res =
                            newType.type == CodeConstants.TYPE_OBJECT
                                    && classname.equals(newType.value);
                    break;
                case Exprent.EXPRENT_VAR:
                    VarExprent varExpr = (VarExprent) expr;
                    if (varExpr.isDefinition()) {
                        VarType varType = varExpr.getVarType();
                        if (classType.equals(varType)
                                || (varType.arrayDim > 0
                                        && classType.value.equals(varType.value))) {
                            res = true;
                        }
                    }
            }
