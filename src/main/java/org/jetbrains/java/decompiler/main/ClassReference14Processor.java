...
public final class ClassReference14Processor {
    ...
    static {
        ...
        invFor.setDescriptor(
                MethodDescriptor.parseDescriptor("(Ljava/lang/String;)Ljava/lang/Class;"));
        ...
        if (!setFound.isEmpty()) {
            for (ClassWrapper wrp : setFound) {
                ...
                wrp.getHiddenMembers()
                        .add(InterpreterUtil.makeUniqueKey(mt.getName(), mt.getDescriptor()));
            }
        }
    }
        ...
        for (MethodWrapper meth : wrapper.getMethods()) {
            ...
            if (root != null) {
                ...
                graph.iterateExprents(
                        exprent -> {
                            for (Entry<ClassWrapper, MethodWrapper> ent :
                                    mapClassMeths.entrySet()) {
            ...
            for (Exprent expr : exprent.getAllExprents()) {
                ...
                if (cl != null) {
                    exprent.replaceExprent(
                            expr,
                            new ConstExprent(
                                    VarType.VARTYPE_CLASS, cl.replace('.', '/'), expr.bytecode));
                    found = true;
                    res = true;
                    break;
                }
                            ...
                            if (fieldnode != null
                                    && fieldnode.classStruct.qualifiedName.equals(
                                            wrapper.getClassStruct()
                                                    .qualifiedName)) { // source class
                                StructField fd =
                                        wrapper.getClassStruct()
                                                .getField(
                                                        field.getName(),
                                                        field.getDescriptor()
                                                                .descriptorString); // FIXME: can be
                                // null! why??
                                            ...
                                            if (invexpr.getClassname()
                                                    ...
                                                    && invexpr.getStringDescriptor()
                                                            .equals(
                                                                    meth.methodStruct
                                                                            .getDescriptor())) {
                                                if (invexpr.getLstParameters().get(0).type
                                                    ...
                                                    wrapper.getHiddenMembers()
                                                            .add(
                                                                    InterpreterUtil.makeUniqueKey(
                                                                            fd.getName(),
                                                                            fd
                                                                                    .getDescriptor())); // hide synthetic field
                                                    return ((ConstExprent)
