...
public class NestedMemberAccess {
            ...
            if ((noSynthFlag || mt.isSynthetic()) && mt.hasModifier(CodeConstants.ACC_STATIC)) {
                if (graph.nodes.size() == 2) { // incl. dummy exit node
                    if (graph.first.exprents.size() == 1) {
                        ...
                        MethodDescriptor mtdesc =
                                MethodDescriptor.parseDescriptor(mt.getDescriptor());
                        ...
                        switch (exprCore.type) {
                            case Exprent.EXPRENT_FIELD:
                                ...
                                if ((parcount == 1 && !fexpr.isStatic())
                                    ...
                                    if (fexpr.getClassname()
                                            .equals(
                                                    node.classStruct
                                                            .qualifiedName)) { // FIXME: check for
                                        // private flag of
                                        // the field
                                        if (fexpr.isStatic()
                                                || (fexpr.getInstance().type == Exprent.EXPRENT_VAR
                                                        && ((VarExprent) fexpr.getInstance())
                                                                        .getIndex()
                                                                == 0)) {
                            ...
                            case Exprent.EXPRENT_FUNCTION:
                                ...
                                if (functionExprent.getFuncType() >= FunctionExprent.FUNCTION_IMM
                                        && functionExprent.getFuncType()
                                                <= FunctionExprent.FUNCTION_PPI) {
                                    if (functionExprent.getLstOperands().get(0).type
                                ...
                                if (asexpr.getLeft().type == Exprent.EXPRENT_FIELD
                                    ...
                                    if ((parcount == 2 && !fexpras.isStatic())
                                        ...
                                        if (fexpras.getClassname()
                                                .equals(
                                                        node.classStruct
                                                                .qualifiedName)) { // FIXME: check
                                            // for private
                                            // flag of the
                                            // field
                                            if (fexpras.isStatic()
                                                    || (fexpras.getInstance().type
                                                                    == Exprent.EXPRENT_VAR
                                                            && ((VarExprent) fexpras.getInstance())
                                                                            .getIndex()
                                                                    == 0)) {
                                                if (((VarExprent) asexpr.getRight()).getIndex()
                                                        == parcount - 1) {
                        ...
                        if (type
                                == MethodAccess
                                        .METHOD) { // FIXME: check for private flag of the method
                                    ...
                                    if (fexpras.getClassname()
                                        ...
                                        if (fexpras.isStatic()
                                            ...
                                            if (((VarExprent) asexpr.getRight()).getIndex()
                                                ...
                                                if (exexpr.getExitType() == ExitExprent.EXIT_RETURN
                                                        && exexpr.getValue() != null) {
                                                    if (exexpr.getValue().type
                                                                    == Exprent.EXPRENT_VAR
                                                            && ((VarExprent) asexpr.getRight())
                                                                            .getIndex()
                                                                    == parcount - 1) {
                                                        type = MethodAccess.FIELD_SET;
                                                    }
                ...
                while (!stack.isEmpty()) { // TODO: replace with interface iterator?
                    ...
                    for (int i = 0; i < nd.exprents.size(); i++) {
                        ...
                        if (exprent.type == Exprent.EXPRENT_INVOCATION) {
                            Exprent ret =
                                    replaceAccessExprent(node, meth, (InvocationExprent) exprent);
    ...
    private Exprent replaceAccessExprent(
            ClassNode caller, MethodWrapper methdest, InvocationExprent invexpr) {
        ClassNode node =
        ...
        switch (type) {
            case FIELD_GET:
                ...
                if (exsource.getValue().type == Exprent.EXPRENT_VAR) { // qualified this
                    ...
                    int index =
                            methdest.counter.getCounterAndIncrement(CounterContainer.VAR_COUNTER);
            ...
            case METHOD:
                ...
                for (int i = 0; i < invret.getLstParameters().size(); i++) {
                    invret.replaceExprent(
                            invret.getLstParameters().get(i),
                ...
                node.getWrapper()
                        ...
                        .add(
                                InterpreterUtil.makeUniqueKey(
                                        invexpr.getName(), invexpr.getStringDescriptor()));
            }
        }
    ...
    private static Exprent replaceFunction(final InvocationExprent invexpr, final Exprent source) {
        FunctionExprent functionExprent =
                (FunctionExprent) ((ExitExprent) source).getValue().copy();
