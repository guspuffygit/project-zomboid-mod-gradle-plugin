...
public class Fernflower implements IDecompiledData {
    ...
    public Fernflower(
        ...
        if (level != null) {
            try {
                logger.setSeverity(
                        IFernflowerLogger.Severity.valueOf(level.toUpperCase(Locale.ENGLISH)));
        ...
        if ("1".equals(properties.get(IFernflowerPreferences.RENAME_ENTITIES))) {
            helper =
                    loadHelper(
                            (String) properties.get(IFernflowerPreferences.USER_RENAMER_CLASS),
                            logger);
        ...
        DecompilerContext context =
                new DecompilerContext(
                        properties, logger, structContext, classProcessor, interceptor);
    ...
    public String getClassEntryName(StructClass cl, String entryName) {
        ...
        if (node.type != ClassNode.CLASS_ROOT) {
            ...
            String simpleClassName =
                    cl.qualifiedName.substring(cl.qualifiedName.lastIndexOf('/') + 1);
    ...
    public String getClassContent(StructClass cl) {
        try {
            ...
            DecompilerContext.getLogger()
                    .writeMessage(
                            "Class " + cl.qualifiedName + " couldn't be fully decompiled.", t);
            return null;
        }
    }
}
