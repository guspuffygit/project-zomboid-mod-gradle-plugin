...
public class MethodProcessorRunnable implements Runnable {
    ...
    public MethodProcessorRunnable(
            StructMethod method,
    ...
    public static RootStatement codeToJava(
            StructMethod mt, MethodDescriptor md, VarProcessor varProc) throws IOException {
        StructClass cl = mt.getClassStruct();

        boolean isInitializer =
                CodeConstants.CLINIT_NAME.equals(mt.getName()); // for now static initializer only
        ...
        if (DecompilerContext.getOption(IFernflowerPreferences.ENSURE_SYNCHRONIZED_MONITOR)) {
            // special case: search for 'synchronized' ranges w/o monitorexit instruction (as
            // generated by Kotlin and Scala)
        ...
        if (ExceptionDeobfuscator.hasObfuscatedExceptions(graph)) {
            ...
            if (!ExceptionDeobfuscator.handleMultipleEntryExceptionRanges(graph)) {
                DecompilerContext.getLogger()
                        .writeMessage(
                                "Found multiple entry exception ranges which could not be splitted",
                                IFernflowerLogger.Severity.WARN);
            }
        ...
        while (true) {
            ...
            // initializer may have at most one return point, so no transformation of method exits
            // permitted
