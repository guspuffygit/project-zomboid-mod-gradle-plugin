            ...
            if (attribute != null) {
                for (StructInnerClassesAttribute.Entry entry : attribute.getEntries()) {
                    if (entry.enclosingName != null
                            && entry.enclosingName.equals(currentClass.qualifiedName)) {
    ...
    public String getShortName(String fullName, boolean imported) {
        ClassNode node =
                DecompilerContext.getClassProcessor()
                        .getMapRootClasses()
                        .get(fullName.replace('.', '/')); // todo[r.sh] anonymous classes?
        ...
        // 3) inner class with the same short name in the current class, a super class, or an
        ...
        if (existsDefaultClass
            ...
            //  don't return full name because if the class is a inner class, full name refers to
            // the parent full name, not the child full name
