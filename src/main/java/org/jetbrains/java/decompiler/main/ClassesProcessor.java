...
public class ClassesProcessor implements CodeConstants {
    ...
    private static class Inner {
        ...
        private static boolean equal(Inner o1, Inner o2) {
            return o1.type == o2.type
                    ...
                    && InterpreterUtil.equalObjects(o1.simpleName, o2.simpleName);
        }
    }
    ...
    public void loadClasses(IIdentifierRenamer renamer) {
        ...
        boolean bDecompileInner =
                DecompilerContext.getOption(IFernflowerPreferences.DECOMPILE_INNER);
        boolean verifyAnonymousClasses =
                DecompilerContext.getOption(IFernflowerPreferences.VERIFY_ANONYMOUS_CLASSES);
            ...
            if (cl.isOwn() && !mapRootClasses.containsKey(cl.qualifiedName)) {
                if (bDecompileInner) {
                    StructInnerClassesAttribute inner =
                            cl.getAttribute(StructGeneralAttribute.ATTRIBUTE_INNER_CLASSES);
                        ...
                        for (StructInnerClassesAttribute.Entry entry : inner.getEntries()) {
                            ...
                            if (savedName != null) {
                                simpleName = savedName;
                                    ...
                                    && renamer != null
                                    && renamer.toBeRenamed(
                                            IIdentifierRenamer.Type.ELEMENT_CLASS,
                                            simpleName,
                                            null,
                                            null)) {
                            ...
                            if (enclosingClass != null
                                ...
                                mapNestedClassReferences
                                        .computeIfAbsent(enclClassName, k -> new HashSet<>())
                                        .add(innerName);
                                // reference to the enclosing class
                                mapEnclosingClassReferences
                                        .computeIfAbsent(innerName, k -> new HashSet<>())
                                        .add(enclClassName);
                            }
                        }
                    }
                ...
                if (!mapInnerClasses.containsKey(ent.getKey())) {
                    ...
                    while (!stack.isEmpty()) {
                        ...
                        if (setNestedClasses != null) {
                            ...
                            StructInnerClassesAttribute inner =
                                    scl.getAttribute(
                                            StructGeneralAttribute.ATTRIBUTE_INNER_CLASSES);
                            ...
                            for (StructInnerClassesAttribute.Entry entry : inner.getEntries()) {
                                ...
                                if (nestedNode == null) {
                                    DecompilerContext.getLogger()
                                            .writeMessage(
                                                    "Nested class " + nestedClass + " missing!",
                                                    IFernflowerLogger.Severity.WARN);
                                    continue;
                                }
                                ...
                                nestedNode.enclosingClasses.addAll(
                                        mapEnclosingClassReferences.get(nestedClass));
    ...
    private static boolean isAnonymous(StructClass cl, StructClass enclosingCl) {
        ...
        StructEnclosingMethodAttribute attribute =
                cl.getAttribute(StructGeneralAttribute.ATTRIBUTE_ENCLOSING_METHOD);
        ...
        for (StructMethod mt : enclosingCl.getMethods()) {
            ...
            try {
                ...
                if (seq != null) {
                    ...
                    for (int i = 0; i < len; i++) {
                        ...
                        switch (instr.opcode) {
                            ...
                            case opc_putstatic:
                                if (cl.qualifiedName.equals(
                                        pool.getLinkConstant(instr.operand(0)).classname)) {
                                    refCounter++;
                                    refNotNew = true;
                                }
                        }
                ...
                String message =
                        "Could not read method while checking anonymous class definition: '"
            ...
            if (refCounter > 1 || refNotNew) {
                String message =
                        "Inconsistent references to the class '"
                                + cl.qualifiedName
    ...
    public static class ClassNode {
        ...
        public ClassNode(
            ...
            lambdaInformation.content_method_key =
                    InterpreterUtil.makeUniqueKey(
                            lambdaInformation.content_method_name,
            ...
            if (!is_method_reference) { // content method in the same class, check synthetic flag
                StructMethod mt =
                        classStruct.getMethod(content_method_name, content_method_descriptor);
            ...
            lambdaInformation.is_content_method_static =
                    (lambdaInformation.content_method_invocation_type
                            == CodeConstants
                                    .CONSTANT_MethodHandle_REF_invokeStatic); // FIXME: redundant?
        }
        ...
        public ClassNode(int type, StructClass classStruct) {
            ...
            simpleName =
                    classStruct.qualifiedName.substring(
                            classStruct.qualifiedName.lastIndexOf('/') + 1);
        }
