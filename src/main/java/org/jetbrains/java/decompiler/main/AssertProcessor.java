...
public final class AssertProcessor {
    ...
    private static final VarType CLASS_ASSERTION_ERROR =
            new VarType(CodeConstants.TYPE_OBJECT, 0, "java/lang/AssertionError");
            ...
            if (wrapper.getStaticFieldInitializers().containsKey(keyField)) {
                ...
                if (fd.hasModifier(CodeConstants.ACC_STATIC)
                    ...
                    if (VarType.VARTYPE_BOOLEAN.equals(fdescr.type)) {
                        Exprent initializer =
                                wrapper.getStaticFieldInitializers().getWithKey(keyField);
                        if (initializer.type == Exprent.EXPRENT_FUNCTION) {
                            ...
                            if (fexpr.getFuncType() == FunctionExprent.FUNCTION_BOOL_NOT
                                    && fexpr.getLstOperands().get(0).type
                                            == Exprent.EXPRENT_INVOCATION) {
                                InvocationExprent invexpr =
                                        (InvocationExprent) fexpr.getLstOperands().get(0);
                                    ...
                                    if (VarType.VARTYPE_CLASS.equals(cexpr.getConstType())) {
                                        ...
                                        while (nd != null) {
                                            if (nd.getWrapper()
                                                    .getClassStruct()
                                                    .qualifiedName
                                                    .equals(cexpr.getValue())) {
                                                break;
                                            }
                                            nd = nd.parent;
                                        }

                                        if (nd != null) { // found enclosing class with the same
                                            // name
                                            return fd;
                                        }
                                    }
    ...
    private static boolean replaceAssertion(
            Statement parent, IfStatement stat, String classname, String key) {
        ...
        Object[] exprres =
                getAssertionExprent(
                        stat.getHeadexprent().getCondition().copy(), classname, key, throwInIf);
    ...
    private static InvocationExprent isAssertionError(Statement stat) {
        ...
        if (expr.type == Exprent.EXPRENT_EXIT) {
            ...
            if (exexpr.getExitType() == ExitExprent.EXIT_THROW
                ...
                if (CLASS_ASSERTION_ERROR.equals(nexpr.getNewType())
    ...
    private static Object[] getAssertionExprent(
            Exprent exprent, String classname, String key, boolean throwInIf) {
    ...
    private static boolean isAssertionField(
            Exprent exprent, String classname, String key, boolean throwInIf) {
        if (throwInIf) {
            if (exprent.type == Exprent.EXPRENT_FUNCTION) {
