...
public class LambdaProcessor {
    ...
    public void processClass(ClassNode node) throws IOException {
        ...
        if (cl.getBytecodeVersion()
                < CodeConstants.BYTECODE_JAVA_8) { // lambda beginning with Java 8
            return;
        }
        ...
        // iterate over code and find invocations of bootstrap methods. Replace them with anonymous
        // classes.
            ...
            if (seq != null && seq.length() > 0) {
                ...
                for (int i = 0; i < len; ++i) {
                    ...
                    if (instr.opcode == CodeConstants.opc_invokedynamic) {
                        LinkConstant invoke_dynamic =
                                cl.getPool().getLinkConstant(instr.operand(0));
                        ...
                        if (lambda_methods.get(invoke_dynamic.index1)) { // lambda invocation found
                            ...
                            MethodDescriptor md =
                                    MethodDescriptor.parseDescriptor(invoke_dynamic.descriptor);

                            String lambda_class_name = md.ret.value;
                            String lambda_method_name = invoke_dynamic.elementname;
                            String lambda_method_descriptor =
                                    ((PrimitiveConstant) bootstrap_arguments.get(2))
                                            .getString(); // method type

                            LinkConstant content_method_handle =
                                    (LinkConstant) bootstrap_arguments.get(1);

                            ClassNode node_lambda =
                                    new ClassNode(
                                            content_method_handle.classname,
                            ...
                            clProcessor
                                    .getMapRootClasses()
                                    .put(node_lambda.simpleName, node_lambda);
                            if (!node_lambda.lambdaInformation.is_method_reference) {
                                mapMethodsLambda.put(
                                        node_lambda.lambdaInformation.content_method_key,
                                        node_lambda.simpleName);
                            }
                        }
                    }
