...
public class ControlFlowGraph implements CodeConstants {
    ...
    private void buildBlocks(InstructionSequence instrseq) {
        ...
        VBStyleCollection<BasicBlock, Integer> colBlocks =
                createBasicBlocks(states, instrseq, mapInstrBlocks);
    ...
    private VBStyleCollection<BasicBlock, Integer> createBasicBlocks(
            short[] startblock,
    ...
    private static void connectBlocks(
            List<BasicBlock> lstbb, Map<Integer, BasicBlock> mapInstrBlocks) {
    ...
    private void setExceptionEdges(
            InstructionSequence instrseq, Map<Integer, BasicBlock> instrBlocks) {
                ...
                while (!stack.isEmpty()) {
                    ...
                    switch (node.getSeq().getLastInstr().opcode) {
                        ...
                        case CodeConstants.opc_ret:
                            ...
                            BasicBlock exit =
                                    blocks.getWithKey(
                                            enter.id + 1); // FIXME: find successor in a better way
    ...
    private int processJsrRanges() {
        ...
        for (int i = 0; i < lstJsr.size(); i++) {
            ...
            for (int j = i + 1; j < lstJsr.size(); j++) {
                ...
                if (!set.contains(arr1.jsr)
                        && !set1.contains(
                                arr.jsr)) { // rang 0 doesn't contain entry 1 and vice versa
                    ...
                    if (!blocks.contains(child)) {
                        if (node != jsr) {
                            for (int k = 0; k < child.getPreds().size(); k++) {
                                if (!DeadCodeHelper.isDominator(
                                        this, child.getPreds().get(k), dom)) {
                                    continue CHILD;
                                }
                            }
                            ...
                            for (int k = 0; k < child.getPredExceptions().size(); k++) {
                                if (!DeadCodeHelper.isDominator(
                                        this, child.getPredExceptions().get(k), dom)) {
                                    continue CHILD;
                                }
                            }
                        }
    ...
    private void splitJsrExceptionRanges(
            Set<BasicBlock> common_blocks, Map<Integer, BasicBlock> mapNewNodes) {
        for (int i = exceptions.size() - 1; i >= 0; i--) {
            ExceptionRangeCFG range = exceptions.get(i);
            List<BasicBlock> lstRange = range.getProtectedRange();
    ...
    private void removeJsr(StructMethod mt) {
        removeJsrInstructions(
                mt.getClassStruct().getPool(), first, DataPoint.getInitialDataPoint(mt));
    }
    ...
    private static void addToReversePostOrderListIterative(
            BasicBlock root, List<? super BasicBlock> lst) {
