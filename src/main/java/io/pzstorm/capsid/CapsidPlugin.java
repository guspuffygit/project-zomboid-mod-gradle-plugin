...
public class CapsidPlugin implements Plugin<Project> {
     ...
     */
    private static void registerDependenciesInEvaluation(
            Project project, DependencyHandler dependencies) {
    ...
    public void apply(Project project) {
        ...
        JavaPluginExtension javaExtension =
                Objects.requireNonNull(extensions.getByType(JavaPluginExtension.class));
                ...
                p -> {
                    ...
                    if (System.getenv("jupiter.functionalTest") == null) {
                        ...
                        tasks.getByName("classes")
                                .dependsOn(tasks.getByName(ZomboidTasks.UPDATE_ZOMBOID_LUA.name));
                    }
                    ...
                    if (capsidExt.isModProject) {
                        ...
                        DistributionContainer distributions =
                                extensions.getByType(DistributionContainer.class);
                        Distribution distribution = distributions.getByName("main");
                        distribution.contents(
                                copy -> {
                                    copy.from(
                                            ProjectProperty.MEDIA_CLASSES_DIR.get(project),
                        ...
                        project.getTasks()
                                ...
                                .dependsOn(
                                        DistributionTasks.MEDIA_CLASSES.name,
                                        DistributionTasks.PROCESS_RESOURCES.name);
                    } else {
                        // register dependencies that are only available during evaluation
                        registerDependenciesInEvaluation(project, dependencies);
