...
public class VmParameter {
     ...
     *
     * <p>Boolean options are used to either enable a feature that is disabled by default or disable
     * a feature that is enabled by default. Such options do not require a parameter.
     *
     * <p>Boolean -XX options are enabled using the plus sign {@code (-XX:+OptionName)} and disabled
     * using the minus sign {@code (-XX:-OptionName)}.
     *
     ...
     * @see <a href="https://docs.oracle.com/javase/8/docs/technotes/tools/windows/java.html">Java
     *     Platform, Standard Edition Tools Reference</a>
     */
    ...
    /**
     * Returns delimiter to be used to separate {@code VMParamter} paths depending on the operating
     * system platform the user is on. Use {@code ;} on Windows and {@code :} on other platforms.
     */
        ...
        String expOptions =
                String.join(
                        ...
                        new String[] {
                            ...
                            formatAdvancedRuntimeOption(
                                    "CreateMinidumpOnCrash", createMinidumpOnCrash),
                            formatAdvancedRuntimeOption(
                                    "OmitStackTraceInFastThrow", omitStackTraceInFastThrow)
                        });
        StringBuilder sb = new StringBuilder();
        sb.append(
                String.format(
                                "%s -Dzomboid.steam=%d -Dzomboid.znetlog=%d %s -Xms%dm -Xmx%dm",
                                isDebug ? "-Ddebug" : "",
                                steamIntegration ? 1 : 0,
                                zNetLog,
                                expOptions,
                                xms,
                                xmx)
